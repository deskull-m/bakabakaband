#include "alliance/alliance-khrone.h"
...
int AllianceKhrone::calcImpressionPoint(PlayerType *creature_ptr) const
{
    int impression = 0;
...
bool AllianceKhrone::isAnnihilated()
{
    return false; // TODO: MonraceList::get_instance().get_monrace(MonraceId::KHORNE_GOD).mob_num == 0;
}

void AllianceKhrone::panishment(PlayerType &player_ptr)
{
    ...
    */
    ...
    if (one_in_(20)) {
        ...
        const auto m_idx = place_monster_one(&player_ptr, m_pos.y, m_pos.x, MonraceId::KHRONE_CHOSEN, PM_ALLOW_GROUP | PM_JURAL);
        if (m_idx) {
            msg_print(_("コーンの選ばれし者があなたを誅すべく追跡してきた！", "\"Hey, let's go!\" Khorne's Chosen is chasing you for revenge!"));
            disturb(&player_ptr, true, true);
            for (int k = 0; k < 3; k++) {
