...
int AllianceTheShire::calcImpressionPoint([[maybe_unused]] const CreatureEntity &creature) const
{
    ...
    if (creature.incident_tree.count(kill_key)) {
        impression -= creature.incident_tree.at(kill_key) * 10; // 1体につき-10
    }

    // 邪悪なモンスター殺害による印象値増加
    const std::string evil_kill_key = "KILL/RACE/EVIL";
    if (creature.incident_tree.count(evil_kill_key)) {
        impression += creature.incident_tree.at(evil_kill_key) * 1; // 1体につき+1
    }

    // 善良なモンスター殺害による印象値減少
    const std::string good_kill_key = "KILL/RACE/GOOD";
    if (creature.incident_tree.count(good_kill_key)) {
        impression -= creature.incident_tree.at(good_kill_key) * 2; // 1体につき-2
    }
