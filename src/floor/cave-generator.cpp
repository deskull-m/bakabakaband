...
static bool make_centers(CreatureEntity &creature, DungeonData *dd_ptr, const DungeonDefinition &dungeon, dt_type *dt_ptr)
{
    dd_ptr->tunnel_fail_count = 0;
    dd_ptr->door_n = 0;
    dd_ptr->tunnel_pos = dd_ptr->centers[dd_ptr->cent_n - 1];
    for (size_t i = 0; i < dd_ptr->cent_n; i++) {
        if (!decide_tunnel_planned_site(creature, dd_ptr, dungeon, dt_ptr, i)) {
            return false;
        }

        make_tunnels(creature, dd_ptr);
        make_walls(creature, dd_ptr, dungeon, dt_ptr);
...
static bool make_one_floor(CreatureEntity &creature, DungeonData *dd_ptr, const DungeonDefinition &dungeon)
{
    ...
    place_cave_contents(creature, dd_ptr, dungeon);
    dt_type tmp_dt;
    dt_type *dt_ptr = initialize_dt_type(&tmp_dt);
    if (!make_centers(creature, dd_ptr, dungeon, dt_ptr)) {
        return false;
    }
...
static bool switch_making_floor(CreatureEntity &creature, DungeonData *dd_ptr, const DungeonDefinition &dungeon)
{
    ...
    if (!make_one_floor(creature, dd_ptr, dungeon)) {
        return false;
    }
...
tl::optional<std::string> cave_gen(CreatureEntity &creature, tl::optional<uint32_t> seed)
{
    ...
    if (dungeon.flags.has(DungeonFeatureType::VESTIGE)) {
        apply_vestige_terrain_replacement(creature);
    }

    // 時空崩壊度に応じて虚空地形を配置
    apply_void_terrain_placement(creature);
    ...
    decide_dungeon_data_allocation(creature, &dd, dungeon);
    if (!allocate_dungeon_data(creature, &dd, dungeon)) {
        return dd.why;
    }
