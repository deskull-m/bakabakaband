...
void add_door(CreatureEntity &creature, const Pos2D &pos)
{
    ...
    if (floor.get_grid(pos_top).is_floor() && floor.get_grid(pos_bottom).is_floor() && floor.get_grid(pos_left).is_outer() && floor.get_grid(pos_right).is_outer()) {
        ...
        place_bold(creature, pos_left.y, pos_left.x, GB_SOLID);
        place_bold(creature, pos_right.y, pos_right.x, GB_SOLID);
    }
    ...
    if (floor.get_grid(pos_top).is_outer() && floor.get_grid(pos_bottom).is_outer() && floor.get_grid(pos_left).is_floor() && floor.get_grid(pos_right).is_floor()) {
        place_secret_door(creature, pos);
        place_bold(creature, pos_top.y, pos_top.x, GB_SOLID);
        place_bold(creature, pos_bottom.y, pos_bottom.x, GB_SOLID);
    }
}
...
void place_secret_door(CreatureEntity &creature, const Pos2D &pos, tl::optional<DoorKind> door_kind_initial)
{
    ...
    place_closed_door(creature, pos, door_kind);
...
void place_closed_door(CreatureEntity &creature, const Pos2D &pos, DoorKind door_kind)
{
    ...
    set_terrain_id_to_grid(creature, pos, tag);
