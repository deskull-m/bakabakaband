...
static void do_cmd_refill_lamp(CreatureEntity &creature)
{
    ...
    const auto &[item, i_idx] = choose_object(creature, q, s, USE_INVEN | USE_FLOOR, FuncItemTester(&ItemEntity::can_refill_lantern));
    if (!item) {
        return;
    }
...
static void do_cmd_refill_torch(CreatureEntity &creature)
{
    ...
    const auto &[item, i_idx] = choose_object(creature, q, s, USE_INVEN | USE_FLOOR, FuncItemTester(&ItemEntity::can_refill_torch));
    if (!item) {
        return;
    }
