...
autotools（Unix 系）では `make check`、Windows では Visual Studio の `BakabakabandTest` プロジェクトで実行します。
...
`VisualStudio/Bakabakaband.sln` の `BakabakabandTest` プロジェクトがテスト実行ファイルを生成します。
ソリューションをビルドすればゲーム本体と一緒にビルドされます。

コマンドラインからビルドする場合は次のとおりです。

```pwsh
MSBuild .\VisualStudio\Bakabakaband.sln /p:Configuration=Debug
.\VisualStudio\Bakabakaband\Debug\bakabakaband-test.exe
```

出力先は構成ごとに分かれます（`Debug` / `English-Debug` / `Release` / `English-Release`）。

```pwsh
.\VisualStudio\Bakabakaband\<構成>\bakabakaband-test.exe
```

Visual Studio の IDE から実行する場合は、`BakabakabandTest` を右クリックして
「スタートアップ プロジェクトに設定」してから開始してください
（既定のスタートアッププロジェクトはゲーム本体の `Bakabakaband` です）。
...
3. **2 つのビルド定義の両方に登録する**
   ...
   **どちらも自動収集はされません。** 登録を忘れたテストはビルドも実行もされないので、
   ...
   `VisualStudio/Bakabakaband/BakabakabandTest.vcxproj` の `<ClCompile>`:
   ...
   Visual Studio 上での表示を整えるため、`BakabakabandTest.vcxproj.filters` にも
...
4. **ビルドして確認する**
   ...
   ビルドし、`bakabakaband-test.exe` を実行します。
...
`BakabakabandCore.lib`（`main-win.cpp` 以外）です。
...
無視されます。autotools には `--whole-archive` という逃げ道があります。**MSVC も
`/WHOLEARCHIVE` を提供していますが、`BakabakabandCore.lib` に適用すると** GUI 専用の
...
- **MSVC の CI がテストを実行するのは Debug 構成だけです。** 手元では 4 構成すべてで
  ビルド・実行できます。Release 系は警告の除外リストが Debug と異なるため、Release で
  だけ壊れる変更は CI をすり抜けます。
