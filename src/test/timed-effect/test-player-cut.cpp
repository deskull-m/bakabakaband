...
TEST_CASE("PlayerCut reports the rank of the given value with its damage and color")
{
    for (const auto &expectation : CUT_RANKS) {
        ...
        CHECK(PlayerCut::get_rank(value) == expectation.rank);
        CHECK(PlayerCut::get_damage(value) == expectation.damage);
        CHECK(std::get<0>(PlayerCut::get_expr(value)) == expectation.color);
    }
}
