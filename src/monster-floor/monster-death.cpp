...
void monster_death(PlayerType *player_ptr, MONSTER_IDX m_idx, bool drop_item, AttributeFlags attribute_flags)
{
    ...
    const auto drop_numbers = decide_drop_numbers(*player_ptr, md_ptr, drop_item, floor.inside_arena);
    floor.object_level = (floor.dun_level + md_ptr->r_ptr->level) / 2;
    drop_items_golds(player_ptr, md_ptr, drop_numbers);
    if ((md_ptr->r_ptr->misc_flags.has_not(MonsterMiscType::QUESTOR)) || AngbandSystem::get_instance().is_phase_out() || (md_ptr->m_ptr->r_idx != MonraceId::MELKO) || md_ptr->cloned) {
        return;
    }
