...
bool artifact_scroll(CreatureEntity &creature)
{
    ...
    const auto &[item, i_idx] = choose_object(creature, q, s, (USE_EQUIP | USE_INVEN | USE_FLOOR | IGNORE_BOTHHAND_SLOT), FuncItemTester(object_is_nameless_weapon_armour));
    if (!item) {
        return false;
    }
    ...
    if (item->is_fixed_or_random_artifact()) {
        ...
        if (item->number > 1) {
            ...
            if (i_idx >= 0) {
                inven_item_increase(creature, i_idx, 1 - (item->number));
            } else {
                floor_item_increase(creature, 0 - i_idx, 1 - (item->number));
            }
        }

        okay = become_random_artifact(creature, item.get(), true);
    }
    ...
    if (record_rand_art) {
        const auto diary_item_name = describe_flavor(creature, *item, OD_NAME_ONLY);
...
bool mundane_spell(CreatureEntity &creature, bool only_equip)
{
    ...
    const auto &[item, i_idx] = choose_object(creature, q, s, (USE_EQUIP | USE_INVEN | USE_FLOOR | IGNORE_BOTHHAND_SLOT), *item_tester);
    if (!item) {
        return false;
    }
