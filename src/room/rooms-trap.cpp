...
bool build_type14(CreatureEntity &creature, DungeonData *dd_ptr)
{
    ...
    const auto center = find_space(creature, dd_ptr, ysize + 2, xsize + 2);
    if (!center) {
        return false;
    }
    ...
    for (auto y = y1 - 1; y <= y2 + 1; y++) {
        place_grid(creature, floor.get_grid({ y, x1 - 1 }), GB_OUTER);
        place_grid(creature, floor.get_grid({ y, x2 + 1 }), GB_OUTER);
    }

    for (auto x = x1 - 1; x <= x2 + 1; x++) {
        place_grid(creature, floor.get_grid({ y1 - 1, x }), GB_OUTER);
        place_grid(creature, floor.get_grid({ y2 + 1, x }), GB_OUTER);
    }
