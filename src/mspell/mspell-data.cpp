...
MSpellMessageData::MSpellMessageData()
    : output([](CreatureEntity &, MONSTER_IDX, MONSTER_IDX, int) { return false; })
{
}

MSpellMessageData::MSpellMessageData(const mspell_cast_msg_blind &msg_string)
    : output([msg_string](CreatureEntity &creature, auto m_idx, auto t_idx, int target_type) {
        return monspell_message(creature, m_idx, t_idx, msg_string, target_type);
    })
{
}
...
MSpellMessageData::MSpellMessageData(const std::string_view &blind, const std::string_view &to_player, const std::string_view &to_monster)
    : output([blind, to_player, to_monster](CreatureEntity &creature, auto m_idx, auto t_idx, int target_type) {
        return monspell_message(creature, m_idx, t_idx, { blind.data(), to_player.data(), to_monster.data() }, target_type);
    })
{
}

MSpellDrsData::MSpellDrsData()
    : execute([](CreatureEntity &, MONSTER_IDX) {})
{
}

MSpellDrsData::MSpellDrsData(std::initializer_list<drs_type> drs)
    : execute([drs = std::vector<drs_type>(drs)](CreatureEntity &creature, auto m_idx) {
        for (auto &d : drs) {
            update_smart_learn(creature, m_idx, d);
        }
    })
{
}
...
MSpellDrsData::MSpellDrsData(const drs_type &drs)
    : execute([drs](CreatureEntity &creature, auto m_idx) { update_smart_learn(creature, m_idx, drs); })
{
}
