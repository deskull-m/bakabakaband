...
{
    auto *creature_ptr = PlayerType::get_instance_ptr();
    const auto is_ready = AngbandWorld::get_instance().character_generated && (creature_ptr != nullptr) && (creature_ptr->get_floor() != nullptr);
    if (!is_ready) {
    ...
    return make_ok_response(id, make_bot_json_snapshot(*creature_ptr, *include_map));
}
