...
bool is_grid_perceivable(const PlayerType &player, const Pos2D &pos)
{
    ...
    return is_visible || (grid.is_view() && (is_glowing || player.has_see_nocto() != 0));
}
...
nlohmann::json make_grid_json(const PlayerType &player, const Pos2D &pos)
{
    ...
    if (grid.is_view() && grid.has_monster()) {
        ...
        if (monster.is_valid() && monster.is_visible_on_map()) {
...
const char *monster_health_band(const CreatureEntity &monster)
{
...
nlohmann::json make_visible_monster_json(short m_idx, const CreatureEntity &monster, bool is_hallucinated)
{
...
nlohmann::json make_visible_monsters_json(const PlayerType &player)
{
    ...
    const auto &floor = *player.get_floor();
    const auto is_hallucinated = player.is_hallucinated();
    for (short m_idx = 1; m_idx < floor.m_max; ++m_idx) {
        ...
        if (!monster.is_valid() || !monster.is_visible_on_map() || !floor.get_grid(monster.get_position()).is_view()) {
            continue;
        }
...
nlohmann::json make_detected_monsters_json(const PlayerType &player)
{
    ...
    for (short m_idx = 1; m_idx < floor.m_max; ++m_idx) {
        ...
        monster_json["esp"] = monster.has_temporary_flag(MonsterTemporaryFlagType::ESP);
        monster_json["detected"] = monster.has_constant_flag(MonsterConstantFlagType::MARK);
...
nlohmann::json make_player_status_json(const CreatureEntity &creature)
{
    return {
        { "blind", creature.is_blind() },
        { "confused", creature.is_confused() },
        { "afraid", creature.is_fearful() },
        { "poisoned", creature.is_poisoned() },
        { "stunned", creature.is_stunned() },
        { "cut", creature.is_cut() },
        { "paralyzed", creature.is_paralyzed() },
        { "hallucinated", creature.is_hallucinated() },
    };
}
...
nlohmann::json make_player_stats_json(const PlayerType &player)
{
    ...
    auto stat = [&player](int index) {
        return nlohmann::json{
            { "cur", player.get_stat_cur(index) },
            { "max", player.get_stat_max(index) },
            { "use", player.get_stat_use(index) },
            { "index", player.get_stat_index(index) },
            { "drained", player.get_stat_cur(index) < player.get_stat_max(index) },
        };
    };
    return {
        { "str", stat(A_STR) },
...
nlohmann::json make_player_immunities_json(PlayerType *player_ptr, const PlayerKnownFlags &flags)
{
    ...
    auto nether = nlohmann::json{
        { "equipment", false },
        { "permanent", CreatureRace(player_ptr).equals(PlayerRaceType::SPECTRE) },
        { "temporary", false },
    };
    return {
...
nlohmann::json make_player_vulnerabilities_json(PlayerType *player_ptr, const PlayerKnownFlags &flags)
{
    ...
    const auto has_element_mutation = player_ptr->has_mutation(PlayerMutationType::VULN_ELEM);
...
{
    if (player.get_food() < PY_FOOD_FAINT) {
        return "fainting";
    }
    if (player.get_food() < PY_FOOD_WEAK) {
        return "weak";
    }
    if (player.get_food() < PY_FOOD_ALERT) {
        return "hungry";
    }
    if (player.get_food() < PY_FOOD_FULL) {
        return "normal";
    }
    if (player.get_food() < PY_FOOD_MAX) {
        return "full";
    }
...
nlohmann::json make_item_json(PlayerType *player_ptr, const ItemEntity &item)
{
    ...
    if (item.ident.has(IdentificationFlag::SENSE)) {
        ...
        if (grid.has_monster()) {
            ...
            if (monster.is_valid() && monster.is_visible_on_map()) {
                ...
                for (const auto i_idx : INVEN_PACK_SLOTS) {
                    const auto &item = monster.inventory[i_idx];
    ...
    BotSnapshotMessages messages_source = BotSnapshotMessages::RECENT_DIFF)
{
    ...
    auto snapshot = nlohmann::json{
        ...
        { "player", {
                        ...
                        { "hp", player.get_current_hp() },
                        { "max_hp", player.get_max_hp() },
                        { "mp", player.get_current_mp() },
                        { "max_mp", player.get_max_mp() },
                        { "exp", player.get_exp() },
                        { "gold", player.get_au() },
                        { "food_state", food_state(player) },
                        { "speed", player.get_speed() },
                        { "recalling", player.get_timed_effect(CreatureTimedEffect::WORD_RECALL) != 0 },
                        { "food_type", enum2i(CreatureRace(player_ptr).food()) },
                        { "class_id", enum2i(player.pclass) },
                        { "race_id", enum2i(player.prace) },
                        { "personality_id", enum2i(player.ppersonality) },
                        { "ac", player.get_dis_ac() + player.get_dis_to_a() },
                        { "skills", {
                                        { "melee", player.get_skill_melee_hit() },
                                        { "shooting", player.get_skill_shoot_hit() },
                                        { "saving", player.get_skill_save() },
                                        { "device", player.get_skill_device() },
                                        { "stealth", player.get_skill_stealth() },
                                        { "two_weapon", player.skill_exp.at(PlayerSkillKindType::TWO_WEAPON) },
                                        { "shield", player.skill_exp.at(PlayerSkillKindType::SHIELD) },
                                    } },
                        { "melee", {
                                       { "main_hand_blows", player.get_num_blow(0) },
                                       { "sub_hand_blows", player.get_num_blow(1) },
                                       { "main_hand_to_h", player.get_dis_to_h(0) },
                                       { "sub_hand_to_h", player.get_dis_to_h(1) },
                                       { "main_hand_to_d", player.get_dis_to_d(0) },
                                       { "sub_hand_to_d", player.get_dis_to_d(1) },
                                   } },
        ...
        { "floor", {
                       ...
                       { "town_id", player.get_town_num() != 0 ? player.get_town_num() - 1 : -1 },
                       { "town_index", player.get_town_num() },
                       ...
                       { "in_town", floor.dun_level == 0 && player.get_town_num() != 0 },
                   } },
        { "progress", {
                          { "recall_dungeon_id", enum2i(player.get_recall_dungeon()) },
                          ...
                          { "recall_depth", dungeon_records.get_record(player.get_recall_dungeon()).get_max_level() },
...
nlohmann::json make_flag_table_json(PlayerType *player_ptr)
{
    ...
    for (const auto flag : flags) {
        ...
        rows.push_back({
            ...
            { "temporary", known_flags.temporary.has(flag) },
...
{
    return (player.has_knowledge(KNOW_STAT) || player.get_stat_max(stat_id) == player.get_stat_max_max(stat_id))
               ? nlohmann::json(player.get_stat_max_max(stat_id))
...
nlohmann::json make_character_json(PlayerType *player_ptr)
{
    ...
    calc_player_two_hands(player, damage.data(), to_h.data());
    int shots = 0;
    int shot_frac = 0;
    const auto &bow = *player.inventory[INVEN_BOW];
    calc_player_shot_params(player, player.inventory[INVEN_BOW].get(), &shots, &shot_frac);
    auto stat_details = nlohmann::json::array();
    for (auto i = 0; i < A_MAX; ++i) {
        stat_details.push_back({
            { "stat_id", i },
            { "top", player.get_stat_top(i) },
            { "maximum", disclosed_stat_maximum(player, i) },
            { "at_maximum", player.get_stat_max(i) == player.get_stat_max_max(i) },
        });
    }
    auto mutations = nlohmann::json::array();
    for (auto i = 0; i < enum2i(PlayerMutationType::MAX); ++i) {
        if (player.has_mutation(i2enum<PlayerMutationType>(i))) {
    ...
    if (bow.is_valid()) {
        shooting_multiplier = (bow.get_arrow_magnification() + (player.has_xtra_might() ? 1 : 0)) * (100 + static_cast<int>(adj_str_td[player.get_stat_index(A_STR)]) - 128);
    }
    return {
        { "skills", {
                        { "thn", player.get_skill_melee_hit() },
                        { "thb", player.get_skill_shoot_hit() },
                        { "sav", player.get_skill_save() },
                        { "dev", player.get_skill_device() },
                        { "stl", player.get_skill_stealth() },
                        { "dis", player.get_skill_disarm() },
                        { "srh", player.get_skill_search() },
                        { "fos", player.get_skill_perception() },
                        { "dig", player.get_skill_dig() },
                    } },
        { "ranged", {
                        { "to_h_b", player.get_to_h_b() },
                        { "shots", shots },
                        { "shot_frac", shot_frac },
                        { "shooting_multiplier", shooting_multiplier },
                        { "see_infra", player.get_infravision() },
                    } },
        { "melee", {
                       { "expected_damage_x100", { damage[0], damage[1] } },
                       { "expected_damage_per_round_x100", { player.get_num_blow(0) * damage[0], player.get_num_blow(1) * damage[1] } },
                       ...
                       { "monk_stance", enum2i(CreatureClass(*player_ptr).get_monk_stance()) },
                   } },
        { "stats", std::move(stat_details) },
        { "age", player.get_age() },
        { "height", player.get_ht() },
        { "weight", player.get_wt() },
        { "alignment", player.alignment },
        { "realms", std::move(realm_json) },
        { "chaos_patron", player.get_patron() },
...
nlohmann::json make_artifacts_knowledge_json(PlayerType *player_ptr, bool identified)
{
    ...
    // bakabakaband の ArtifactRecords は id 集約メソッドを持たないため、ArtifactList を走査して
    // 名称既知 (get_known) / 生成済 (get_generated) で絞り込む。
    for (const auto &[id, artifact] : artifacts) {
        const auto disclosed = identified ? records.get_generated(id) : records.get_known(id);
        if (!disclosed) {
            continue;
        }
...
nlohmann::json make_knowledge_json(PlayerType *player_ptr, BotKnowledgeCategory category)
{
    ...
    // bakabakaband の do_cmd_knowledge メニューキー配置に合わせる (上流と異なる)。
    // ARTIFACTS_IDENTIFIED は bakabakaband に個別メニューが無いため KNOWN と同じ '1' を割当てる。
    static constexpr const char keys[] = { '1', '1', '2', '3', '4', '7', '9', '0', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', '8', 'k' };
    ...
    case BotKnowledgeCategory::SELF_INFO:
        result["life_rating"] = player_ptr->has_knowledge(KNOW_HPRATE) ? nlohmann::json(player_ptr->calc_life_rating()) : nlohmann::json(nullptr);
        for (auto i = 0; i < A_MAX; ++i) {
    ...
    case BotKnowledgeCategory::MUTATIONS:
        for (auto i = 0; i < enum2i(PlayerMutationType::MAX); ++i) {
            if (player_ptr->has_mutation(i2enum<PlayerMutationType>(i))) {
                rows.push_back(i);
            }
        }
    ...
    case BotKnowledgeCategory::VIRTUES:
        for (const auto &[virtue_type, value] : player_ptr->virtues) {
            const auto name_it = virtue_names.find(virtue_type);
            const auto name = name_it != virtue_names.end() ? to_json_utf8(name_it->second) : _("不明", "Oops. No info");
            rows.push_back({ { "id", enum2i(virtue_type) }, { "name", name },
                { "value", value } });
        }
    ...
    case BotKnowledgeCategory::PETS:
        for (auto i = 1; i < player_ptr->get_floor()->m_max; ++i) {
            const auto &monster = player_ptr->get_floor()->m_list[i];
            if (monster.is_valid() && monster.is_pet()) {
                rows.push_back({ { "index", i }, { "race_id", enum2i(monster.get_r_idx()) },
 ...
 */
nlohmann::json make_bot_json_snapshot(CreatureEntity &creature, bool include_map)
{
