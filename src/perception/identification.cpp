...
bool screen_object(CreatureEntity &creature, const ItemEntity &item, BIT_FLAGS mode)
{
    ...
    if (flags.has(TR_INVEN_ACTIVATE)) {
        info[i++] = _("始動したときの効果...", "It can be activated for...");
        info[i++] = activation_explanation(&item);
    } else if (flags.has(TR_ACTIVATE)) {
        info[i++] = _("始動したときの効果...", "It can be activated for...");
        info[i++] = activation_explanation(&item);
        info[i++] = _("...ただし装備していなければならない。", "...if it is being worn.");
    }

    if (flags.has(TR_NASTY)) {
    ...
    if ((flags.has(TR_NASTY_AGGRAVATE)) || item.curse_flags.has(CurseTraitType::NASTY_AGGRAVATE)) {
    ...
    if (!is_fake_object) {
        ...
        item_name = describe_flavor(creature, item, (OD_NAME_ONLY | OD_STORE | OD_OMIT_MONRACE));
    }
