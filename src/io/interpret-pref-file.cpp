 ...
 */
static void interpret_xy_token(CreatureEntity &creature, std::string_view buf)
{
...
static bool interpret_z_token(std::string_view line)
{
    ...
    for (const auto &[name, num] : gf_descriptions) {
        ...
        gf_colors[num] = strtol(splits[2].c_str(), nullptr, 0);
        return true;
    }
 ...
 */
static int interpret_pref_line(CreatureEntity &creature, std::string_view buf)
{
    switch (buf[0]) {
    case 'H':
        /* Process "H:<history>" */
...
int interpret_pref_file(CreatureEntity &creature, std::string_view buf)
{
    ...
    try {
        return interpret_pref_line(creature, buf);
