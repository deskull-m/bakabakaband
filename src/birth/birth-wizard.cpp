...
static void display_initial_options(PlayerType *player_ptr)
{
    ...
    for (int i = 0; i < A_MAX; i++) {
        adj[i] = player_ptr->rp_ptr->r_adj[i] + cp_ptr->c_adj[i] + ap_ptr->a_adj[i];
    }
    ...
    put_str("HD ", 6, 40);
    const auto hd = format("%2d", player_ptr->rp_ptr->r_mhp + cp_ptr->c_mhp + ap_ptr->a_mhp);
    ...
    if (PlayerClass(player_ptr).equals(PlayerClassType::BERSERKER)) {
        ...
        stealth = format("%+2d", player_ptr->rp_ptr->r_stl + cp_ptr->c_stl + ap_ptr->a_stl);
    }
