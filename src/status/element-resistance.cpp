...
bool set_oppose_fire(CreatureEntity &creature, TIME_EFFECT v, bool do_dec)
{
    ...
    if ((CreatureRace(&creature).equals(PlayerRaceType::BALROG) && (creature.get_level() > 44)) || (creature.get_mimic_form() == MimicKindType::DEMON)) {
        v = 1;
    }
    if (v) {
...
bool is_oppose_fire(CreatureEntity &creature)
{
    ...
    can_oppose_fire |= (CreatureRace(&creature).equals(PlayerRaceType::BALROG) && creature.get_level() > 44);
