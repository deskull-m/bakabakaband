...
bool BadStatusSetter::set_blindness(const TIME_EFFECT tmp_v)
{
    ...
    auto &blindness = player_ptr->effects()->blindness();
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_blindness(player_ptr->effects()->blindness().current() + tmp_v);
}
...
bool BadStatusSetter::set_confusion(const TIME_EFFECT tmp_v)
{
    ...
    player_ptr->effects()->confusion().set(v);
    rfu.set_flag(MainWindowRedrawingFlag::TIMED_EFFECT);
    if (!notice) {
        return false;
    }
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_confusion(player_ptr->effects()->confusion().current() + tmp_v);
}
...
bool BadStatusSetter::set_poison(const TIME_EFFECT tmp_v)
{
    ...
    auto &player_poison = player_ptr->effects()->poison();
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_poison(player_ptr->effects()->poison().current() + tmp_v);
}
...
bool BadStatusSetter::set_fear(const TIME_EFFECT tmp_v)
{
    ...
    auto &fear = player_ptr->effects()->fear();
    if (v > 0) {
        if (!fear.is_fearful()) {
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_fear(player_ptr->effects()->fear().current() + tmp_v);
}
...
bool BadStatusSetter::set_paralysis(const TIME_EFFECT tmp_v)
{
    ...
    auto &paralysis = player_ptr->effects()->paralysis();
    if (v > 0) {
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_paralysis(player_ptr->effects()->paralysis().current() + tmp_v);
}
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->hallucination(player_ptr->effects()->hallucination().current() + tmp_v);
}
...
bool BadStatusSetter::set_deceleration(const TIME_EFFECT tmp_v, bool do_dec)
{
    ...
    auto &deceleration = player_ptr->effects()->deceleration();
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_deceleration(player_ptr->effects()->deceleration().current() + tmp_v, do_dec);
}
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_stun(player_ptr->effects()->stun().current() + tmp_v);
}
...
bool BadStatusSetter::set_cut(const TIME_EFFECT tmp_v)
{
    ...
    if (CreatureRace(player_ptr).has_cut_immunity()) {
        v = 0;
    }

    auto notice = this->process_cut_effect(v);
    player_ptr->effects()->cut().set(v);
    if (!notice) {
        return false;
    }
...
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    return this->set_cut(player_ptr->effects()->cut().current() + tmp_v);
}

bool BadStatusSetter::process_stun_effect(const short v)
{
    auto *player_ptr = dynamic_cast<PlayerType *>(&this->creature);
    auto old_rank = player_ptr->effects()->stun().get_rank();
...
void BadStatusSetter::stop_blooding(const PlayerCutRank new_rank)
{
    ...
    auto blood_stop_mes = CreatureRace(player_ptr).equals(PlayerRaceType::ANDROID)
