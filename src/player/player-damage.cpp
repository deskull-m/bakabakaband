...
#ifdef JP
            const auto note = format("%sで%sに殺されて飽きた", place.data(), player_ptr->died_from.data());
#else
            const auto note = format("Tired of being killed by %s in %s.", player_ptr->died_from.data(), place.data());
#endif
