...
void set_breath_types(CreatureEntity &creature, lore_type *lore_ptr)
{
    ...
    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_ACID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_ACID, _("酸%s", "acid%s"), TERM_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_ELEC)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_ELEC, _("稲妻%s", "lightning%s"), TERM_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_FIRE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_FIRE, _("火炎%s", "fire%s"), TERM_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_COLD)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_COLD, _("冷気%s", "frost%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_POIS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_POIS, _("毒%s", "poison%s"), TERM_L_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_NETH)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_NETH, _("地獄%s", "nether%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_LITE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_LITE, _("閃光%s", "light%s"), TERM_YELLOW);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_DARK)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_DARK, _("暗黒%s", "darkness%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_CONF)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_CONF, _("混乱%s", "confusion%s"), TERM_L_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_SOUN)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_SOUN, _("轟音%s", "sound%s"), TERM_ORANGE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_CHAO)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_CHAO, _("カオス%s", "chaos%s"), TERM_VIOLET);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_DISE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_DISE, _("劣化%s", "disenchantment%s"), TERM_VIOLET);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_NEXU)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_NEXU, _("因果混乱%s", "nexus%s"), TERM_VIOLET);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_TIME)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_TIME, _("時間逆転%s", "time%s"), TERM_L_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_INER)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_INER, _("遅鈍%s", "inertia%s"), TERM_SLATE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_GRAV)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_GRAV, _("重力%s", "gravity%s"), TERM_SLATE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_SHAR)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_SHAR, _("破片%s", "shards%s"), TERM_L_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_PLAS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_PLAS, _("プラズマ%s", "plasma%s"), TERM_L_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_FORC)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_FORC, _("フォース%s", "force%s"), TERM_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_MANA)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_MANA, _("魔力%s", "mana%s"), TERM_L_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_NUKE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_NUKE, _("放射性廃棄物%s", "toxic waste%s"), TERM_L_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_DISI)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_DISI, _("分解%s", "disintegration%s"), TERM_SLATE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_VOID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_VOID, _("虚無%s", "void%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_ABYSS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_ABYSS, _("深淵%s", "abyss%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_FECES)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_FECES, _("糞便%s", "feces%s"), TERM_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BR_SPIDER_STRING)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BR_SPIDER_STRING, _("蜘蛛糸%s", "spider string%s"), TERM_WHITE);
    }
}
...
void set_ball_types(CreatureEntity &creature, lore_type *lore_ptr)
{
    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_ACID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_ACID, _("アシッド・ボール%s", "produce acid balls%s"), TERM_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_ELEC)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_ELEC, _("サンダー・ボール%s", "produce lightning balls%s"), TERM_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_FIRE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_FIRE, _("ファイア・ボール%s", "produce fire balls%s"), TERM_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_COLD)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_COLD, _("アイス・ボール%s", "produce frost balls%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_POIS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_POIS, _("悪臭雲%s", "produce poison balls%s"), TERM_L_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_NETH)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_NETH, _("地獄球%s", "produce nether balls%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_WATE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_WATE, _("ウォーター・ボール%s", "produce water balls%s"), TERM_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_NUKE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_NUKE, _("放射能球%s", "produce balls of radiation%s"), TERM_L_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_MANA)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_MANA, _("魔力の嵐%s", "invoke mana storms%s"), TERM_L_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_DARK)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_DARK, _("暗黒の嵐%s", "invoke darkness storms%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_LITE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_LITE, _("スターバースト%s", "invoke starburst%s"), TERM_YELLOW);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_CHAO)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_CHAO, _("純ログルス%s", "invoke raw Logrus%s"), TERM_VIOLET);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_VOID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_VOID, _("虚無の嵐%s", "invoke void storms%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_ABYSS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_ABYSS, _("深淵の嵐%s", "invoke abyss storms%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_METEOR)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_METEOR, _("メテオスウォーム%s", "invoke meteor swarm%s"), TERM_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BA_GRAVITY)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BA_GRAVITY, _("超重力%s", "invoke gravity storms%s"), TERM_SLATE);
    }
}
...
void set_particular_types(CreatureEntity &creature, lore_type *lore_ptr)
{
    ...
    if (lore_ptr->ability_flags.has(MonsterAbilityType::PSY_SPEAR)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::PSY_SPEAR, _("光の剣%s", "psycho-spear%s"), TERM_YELLOW);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::DRAIN_MANA)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::DRAIN_MANA, _("魔力吸収%s", "drain mana%s"), TERM_SLATE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::MIND_BLAST)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::MIND_BLAST, _("精神攻撃%s", "cause mind blasting%s"), TERM_L_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BRAIN_SMASH)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BRAIN_SMASH, _("脳攻撃%s", "cause brain smashing%s"), TERM_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::CAUSE_1)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::CAUSE_1, _("軽傷＋呪い%s", "cause light wounds and cursing%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::CAUSE_2)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::CAUSE_2, _("重傷＋呪い%s", "cause serious wounds and cursing%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::CAUSE_3)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::CAUSE_3, _("致命傷＋呪い%s", "cause critical wounds and cursing%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::CAUSE_4)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::CAUSE_4, _("秘孔を突く%s", "cause mortal wounds%s"), TERM_L_WHITE);
    }
}
...
void set_bolt_types(CreatureEntity &creature, lore_type *lore_ptr)
{
    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_ACID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_ACID, _("アシッド・ボルト%s", "produce acid bolts%s"), TERM_GREEN);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_ELEC)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_ELEC, _("サンダー・ボルト%s", "produce lightning bolts%s"), TERM_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_FIRE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_FIRE, _("ファイア・ボルト%s", "produce fire bolts%s"), TERM_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_COLD)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_COLD, _("アイス・ボルト%s", "produce frost bolts%s"), TERM_L_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_NETH)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_NETH, _("地獄の矢%s", "produce nether bolts%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_WATE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_WATE, _("ウォーター・ボルト%s", "produce water bolts%s"), TERM_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_MANA)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_MANA, _("魔力の矢%s", "produce mana bolts%s"), TERM_L_BLUE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_PLAS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_PLAS, _("プラズマ・ボルト%s", "produce plasma bolts%s"), TERM_L_RED);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_ICEE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_ICEE, _("極寒の矢%s", "produce ice bolts%s"), TERM_WHITE);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_VOID)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_VOID, _("ヴォイド・ボルト%s", "produce void bolts%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_ABYSS)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_ABYSS, _("アビス・ボルト%s", "produce abyss bolts%s"), TERM_L_DARK);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_METEOR)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_METEOR, _("メテオストライク%s", "produce meteor strikes%s"), TERM_UMBER);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_LITE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::BO_LITE, _("スターライトアロー%s", "produce starlight arrow%s"), TERM_YELLOW);
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::MISSILE)) {
        add_lore_of_damage_skill(creature, lore_ptr, MonsterAbilityType::MISSILE, _("マジックミサイル%s", "produce magic missiles%s"), TERM_SLATE);
    }
}
