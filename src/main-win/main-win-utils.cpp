 ...
 * 既に接続済みの場合に呼んでも問題は無い。
 *
 * ただし「Bakabakaband.exe --control-port=9000 2>err.log」のように標準エラー出力の
...
void attach_console()
{
    ...
    if (freopen_s(&stream, "CONOUT$", "w", stderr) != 0) {
        ...
        ::OutputDebugStringA("bakabakaband: failed to attach stderr\n");
    }
}
