...
void set_race_ability_info(CreatureEntity &creature, self_info_type *self_ptr)
{
    ...
    case PlayerRaceType::CYCLOPS:
        if (creature.level >= 20) {
            ...
            const auto dam = (3 * creature.level) / 2;
    ...
    case PlayerRaceType::KLACKON:
        if (creature.level >= 9) {
            ...
            const auto dam = creature.level;
            self_ptr->info_list.push_back(format(fmt, dam));
        }

        break;
    case PlayerRaceType::KOBOLD:
        if (creature.level >= 12) {
    ...
    case PlayerRaceType::DARK_ELF:
        if (creature.level >= 2) {
            ...
            const auto dam = 3 + ((creature.level - 1) / 5);
            self_ptr->info_list.push_back(format(fmt, dam));
        }
    ...
    case PlayerRaceType::DRACONIAN: {
        ...
        const auto dam = 2 * creature.level;
        const auto cost = creature.level;
    ...
    case PlayerRaceType::VAMPIRE:
        if (creature.level >= 2) {
            ...
            const auto dam = creature.level * 2;
            const auto cost = 1 + (creature.level / 3);
            self_ptr->info_list.push_back(format(fmt, dam, cost));
        }
    ...
    case PlayerRaceType::BALROG: {
        ...
        const auto dam = 3 * creature.level;
        const auto cost = 10 + creature.level / 3;
        self_ptr->info_list.push_back(format(fmt, dam, cost));
        break;
    }
    ...
    case PlayerRaceType::ANDROID:
        if (creature.level < 10) {
            ...
            const auto dam = (creature.level + 1) / 2;
            self_ptr->info_list.push_back(format(fmt, dam));
            ...
            const auto dam = creature.level * 5;
            self_ptr->info_list.push_back(format(fmt, dam));
        }
