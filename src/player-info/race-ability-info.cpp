...
void set_race_ability_info(PlayerType *player_ptr, self_info_type *self_ptr)
{
    ...
    case PlayerRaceType::CYCLOPS:
        if (player_ptr->level >= 20) {
            ...
            const auto dam = (3 * player_ptr->level) / 2;
    ...
    case PlayerRaceType::KLACKON:
        if (player_ptr->level >= 9) {
            ...
            const auto dam = player_ptr->level;
            self_ptr->info_list.push_back(format(fmt, dam));
        }

        break;
    case PlayerRaceType::KOBOLD:
        if (player_ptr->level >= 12) {
    ...
    case PlayerRaceType::DARK_ELF:
        if (player_ptr->level >= 2) {
            ...
            const auto dam = 3 + ((player_ptr->level - 1) / 5);
            self_ptr->info_list.push_back(format(fmt, dam));
        }
    ...
    case PlayerRaceType::DRACONIAN: {
        ...
        const auto dam = 2 * player_ptr->level;
        const auto cost = player_ptr->level;
    ...
    case PlayerRaceType::VAMPIRE:
        if (player_ptr->level >= 2) {
            ...
            const auto dam = player_ptr->level * 2;
            const auto cost = 1 + (player_ptr->level / 3);
            self_ptr->info_list.push_back(format(fmt, dam, cost));
        }

        break;
    case PlayerRaceType::SPECTRE:
        if (player_ptr->level >= 4) {
    ...
    case PlayerRaceType::ANDROID:
        if (player_ptr->level < 10) {
            ...
            const auto dam = (player_ptr->level + 1) / 2;
            self_ptr->info_list.push_back(format(fmt, dam));
            ...
            const auto dam = player_ptr->level * 5;
            self_ptr->info_list.push_back(format(fmt, dam));
        }
