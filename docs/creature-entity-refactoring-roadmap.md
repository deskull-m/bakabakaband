...
| C1 | 種族・職業の顕在化（`prace`/`pclass` 付与） | ✅ cache 有 | 中 | 高 | ✅ 1(付与)/2(基本5)/3(二次7)/4(恐怖)/5(自由行動)/6(光闇) 完了 |
...
**第7弾以降（未着手）:** 職業特典・種族の非耐性特典（ESP・赤外線視・stat 補正等）。
ESP/赤外線視はモンスター AI 索敵の新規実装が要る（C3 と同課題）ため大。メンテナの
バランス判断のもと段階導入する。**残る主要属性耐性はほぼ反映済**（基本5・二次7・
光闇・恐怖・自由行動。未反映は time/gravity/nexus 系の一部特殊属性のみ）。
