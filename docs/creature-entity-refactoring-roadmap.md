...
| ESP/知覚 `esp_*` | ✅ | 🟡 AI が読むが付与機構なし | **C3（最優先）** |
...
| C3 | ESP のモンスター付与 | ✅ AI 読取済 | 小 | 高 | どの種族に何の ESP を持たせるか |
...
**基盤:** レベルアップ・HP 成長は第 4 段で稼働（`try_monster_level_up()` /
`grow_hp_table_to_level()`）。ただし成長するのは HP と実効レベルのみ。
`stat_cur/stat_max` は生成後不変、`weapon_exp/skill_exp` はモンスター未使用。

**作業:** レベルアップ時に (a) 能力値を成長式で加算、(b) 攻撃・詠唱で
`weapon_exp/skill_exp` を蓄積し命中/ダメージへ反映、のいずれか/両方。

**デザイン判断（要メンテナ）:**
- 成長式（レベルあたり何点・上限）。HP 成長が「基準レベルの 2 倍で約 2 倍」
  に制限されている既存方針と整合させる。
- 熟練度をモンスター戦闘にどう効かせるか（命中式は B3 で共通化済のため差込口は明確）。
- JSON オプトイン（`"grows_stats": true` 等）で対象を限定。

**工数:** 中。**価値:** 中。**リスク:** 中（際限ない成長は暴走要因、上限必須）。

---

## 提案 C3: ESP のモンスター付与（最優先の着手候補）

**基盤:** ESP フラグは `CreatureEntity` 上にあり、**モンスター AI
（`monster-update.cpp`）が既にターゲット選定で読んでいる**（`is_player()`
ガード無し・UI 非依存）。障壁が最も低い系統。

**作業:** 特定モンスター（幽霊・テレパス種）に ESP フラグを付与する機構を
追加（JSON `"esp": ["UNDEAD", "EVIL"]` 等）。既存 AI がそれを読んで壁越し
知覚・索敵に反映する。モンスター対モンスターの知覚へも自然に拡張。

**デザイン判断（要メンテナ）:** どの種族に何の ESP を持たせるか（データ作業）。

**工数:** 小。**価値:** 高（既に半分実装済・非対称知覚は AI に深みを与える）。
**リスク:** 低。→ **C トラックの最初の着手候補として最適。**

---

## 提案 C4: MP 消費詠唱

**基盤:** `current_mp`/`max_mp` は共通フィールド（提案 6）。モンスターも
pool を持てるが現状消費しない（mspell は無コスト）。

**作業:** モンスター呪文詠唱（mspell）に MP コストを導入し、枯渇時は詠唱不可に。

**デザイン判断（要メンテナ）:** コストモデル（呪文毎の消費量）、枯渇時挙動
（近接に切替 等）、MP 回復（自然回復は提案 8 で共通化済）。JSON オプトイン推奨。

**工数:** 中。**価値:** 中（資源管理する賢いモンスター）。**リスク:** 中
（既存モンスターの詠猛度が下がりバランス変動、オプトインで緩和）。
...
## 提案 C7: class_specific_data の限定運用

**基盤:** variant は共通フィールドだが `pclass=NONE` のモンスターは
`no_class_specific_data`。大半のクラスデータ（鍛冶の essence・魔道具の
チャージ等）はプレイヤーのインベントリ/UI 前提。

**作業:** 汎用性のあるクラスデータ（青魔道士の学習呪文・侍/忍者/僧の構え）
に限りモンスター運用を検討。C1（pclass 付与）が前提。

**デザイン判断（要メンテナ）:** 対象クラスの選定。**工数:** 中。**価値:** 低〜中。
→ C1 の後、必要性が出たら。
...
**C3（ESP）→ C1（種族・職業）→ C2（能力成長）→ C4（MP 詠唱）→ C5（突然変異）
→ C6（realm 詠唱）→ C7（class data）**。C8 は見送り。C0 は随時（低優先、
まず CLAUDE.md の exp 保存記述の訂正のみ先行可）。

C3 は障壁が最も低く（AI 既読）価値が高いため最初に置く。C1 は最大の価値
（種族・職業概念の開放）だがバランス判断を伴うため JSON オプトインで段階導入。
C6 は二系統統合のアーキテクチャ判断を要するため後半。

**全 C 提案に共通する進め方:** ①汎用機構をコードに実装 → ②JSON で 1 体に
オプトイン付与 → ③実機で挙動・バランス確認 → ④メンテナ判断で横展開。
A/B と違い挙動が変わるため、着手時は必ず対象範囲と数値をメンテナに確認する。
