...
**（着手時点で C3 の基盤誤認が判明したため再ランク）**
**C1（種族・職業）→ C2（能力成長）→ C4（MP 詠唱）→ C5（突然変異）→
C3（ESP・要新規 AI）→ C6（realm 詠唱）→ C7（class data）**。C8 は見送り。
C0 は随時（低優先、CLAUDE.md の exp 保存記述の訂正は完了済）。

当初 C3（ESP）を「AI 既読で最優先」としたが、実コード検証で `esp_*` は
プレイヤーのテレパシー専用でモンスター AI は読まないと判明し、新規 AI が要る
中〜大タスクへ降格。代わりに **C1（種族・職業の顕在化）を最初の実装候補**とする
（`equivalent_player_races/classes` cache と `player_birth_as_monster` の
マッピングという実在の基盤があり、JSON オプトイン＋段階導入でバランスを制御できる）。
