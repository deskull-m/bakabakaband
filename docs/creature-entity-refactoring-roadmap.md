...
| [51](#提案-51-残り-public-bool-フィールドの-private-化--完了) | 残り public bool フィールドの private 化 (A-1 第1弾) | ✅ 完了 | counter / select_ring_slot / no_flowed / hack_mutation / invoking_midnight_curse |
...
| B3 | モンスター命中判定 2 関数の統合 | 攻撃 | 中 | 低 | 計画 |
...
## 提案 B2: ターン・エネルギー消費カーネルの共通化

**現状の分離:** プレイヤー (`player-processor.cpp:140-142`, `260`) とモンスター
(`monster-processor.cpp:1610-1615`) が、ほぼ同一の
「`sub_energy_need(speed_to_energy(get_speed()))` → `energy_need > 0` なら手番なし →
手番消化後に `ENERGY_NEED()` 加算」をそれぞれ手書きしている。`speed_to_energy()`
(`speed-table.cpp`) と `ENERGY_NEED()` マクロは既に共用だが、消費ロジック自体は重複。

**真に共有可能:** 速度→エネルギー減算と「手番が来たか」のゲート判定
(`get_energy_need() > 0`)。

**真の分岐:** プレイヤーは手番消化後の加算が `energy_use * ENERGY_NEED()/100`
（行動コスト依存）かつ load 時の前段減算がある。モンスターは一律 `ENERGY_NEED()`、
騎乗時は搭乗者速度を使う。

**提案プリミティブ:** `CreatureEntity::tick_action_energy()`（速度減算）と
`CreatureEntity::has_turn()`（`get_energy_need() <= 0`）の薄い共通メソッド。
加算側は型固有のまま残す。**critical path のため挙動完全不変を厳守し、
ターン消費タイミングの同一性を実機で確認すること。**

**規模/価値:** 小（重複は十数行）。価値は中（タイミング計算の単一真実源化）。

---

## 提案 B3: モンスター命中判定 2 関数の統合
...
**提案:** `check_hit_from_monster_to_monster(power, level, ac, stun)` を素の
カーネルとし、`check_hit_from_monster_to_player()` は AC（+SUIKEN 補正）を
算出してこれに委譲する薄いラッパへ。挙動不変。

**規模/価値:** 小・低リスク（純粋計算、目標の AC 解決のみ差分）。重複した
乱数式が 1 本化され、将来の命中式調整が 1 箇所で済む。**B トラックの最初の
着手候補として最適**（影響範囲が狭く検証容易）。
...
## 提案 B6: `is_player()` 真分岐の限定的 virtual 化

**調査結果:** `is_player()` は src/ 全体で約 102 サイト。内訳は
**(a) 型契約の早期 return ガード ~57（アーキテクチャ的に正当、現状維持）**、
(b) 真の振る舞い分岐 ~35、(c) 冗長なローカルキャッシュ ~5。提案 35 の
「大半は型契約ガード」という結論を再確認した。(b) のうち機械的削減で価値が
出るのは少数。

**着手候補（高 ROI な (b) のみ）:**
- **呪い効果分岐** (`effect-monster-curse.cpp:14/30/47/64`): 4 サイトが
  `if (is_player()) {プレイヤー固有} else {モンスター}` の同型。
  `virtual apply_curse_effect(...)` への寄せでロジック集約。
- **撃破時のランダムアーティファクト生成** (`special-death-switcher.cpp`):
  `if (killer.is_player())` ガードを `virtual on_defeat_monster(...)` 等へ。

**見送り:** 単発サイトの SpellHex アクセス・統計記録・テキスト三項などは
抽象化コストが削減量を上回るため現状維持。

**規模/価値:** 小（2〜3 個の小さな virtual）。リスク低〜中。

---

## 提案 B7: テレポート先判定の共通化

**現状の分離:** `cave_player_teleportable_bold()` と
`cave_monster_teleportable_bold()` が別実装 (`spells-teleport.cpp`)。
さらに `teleport_player_aux()` (`:279`) と `teleport_away()` (`:107`) も別経路。

**提案:** 配置可否判定を `is_teleportable(const CreatureEntity&, const Pos2D&,
teleport_flags)` に寄せる。ただしプレイヤー固有ルール（アンチテレポート、
グリッド種別）とモンスター固有ルールに微妙な差があるため、まず 2 関数の
差分を精査してから共通化すること。

**規模/価値:** 中。リスク中（移動は副作用が広い）。B トラック後半に回す。
...
影響範囲の狭さと検証容易性から **B3 → B5 → B2 → B4 → B6 → B7** を推奨。
B3（命中判定統合）は純粋計算で最も安全なため最初に置く。critical path に
触れる B2 と副作用の広い B7 は、足場が固まった後半に回す。B8 は着手しない。
