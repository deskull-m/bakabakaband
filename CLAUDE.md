...
- **これは実挙動を変える (小幅なバランス変更)**。`INVEN_OUTER` (体の上) は武装度
  ...
  から移した 43 体はクローク 1 枠ぶんの AC を失う (充填順は 胴 → 利き手 → 頭 →
...
- **適用済み: 46 体**。
  ...
  | `H` | ハーピー 2 体 | 2 | HUMANOID | **クローク枠を失う** |
  | `V` | 吸血鬼の使者『スリングウェシル』 | 1 | HUMANOID | **クローク枠を失う** |
  | `B` | パズズ / カイム / ベルカの鴉人 | 3 | BIPEDAL | **利き手・逆手・腕を得る** |

  - **`B` の 3 体だけは強化側**。いずれも `AVIAN` 移行時に「解説文が人間型の腕・体を
    明言している」として除外した個体で (パズズ=「羽を持った**人間型**の
    モンスター」、カイム=「翼から生えた**二本の腕**を持つ…**魔法のサーベルを
    携えている**」、ベルカの鴉人=「細長い**人間の体**に鴉の頭と翼」)、
    `BIPEDAL` のままでは武器を持てず設定と食い違っていた。**本体構造は
    その積み残しの受け皿**でもある。
