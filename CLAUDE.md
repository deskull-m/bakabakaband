...
- **第 2〜4 弾で残りを一巡 (85 体)**。第 2 弾 = Lv0-29 の 22 体、第 3 弾 = Lv30-49 の
  ...
  - **`INSTA_ART` のベースアイテム (☆専用で通常生成されない品) は装備させない**。
    ...
    **生成すれば必ず☆になる品**であり、かつ**グロンドは『混沌のサーペント』(862) の
    ...
    「偽グロンド」になる)。代わりに本件で新設した
    **黄金の槍 (id 831, tval 22 POLEARM, sval 31, 2d9, 1800G, lv55)** を持たせている
    (既存の固定アーティファクト「冥王メルコールの大槍」(artifact 269) の解説文が
    「今黄金に輝いている」と述べる、メルコールのもう一つの得物に対応する)。
  - **一方「たまたま☆のベースでもある通常アイテム」は気にせず付与してよい**。
    ...
    **☆にはならず素の鉄冠として装備される** (☆側も通常どおり入手できる)。
    ...
    クローク / 軍服 も同様で、**`equip_kind` 110 エントリ中 82 が何らかの☆のベース**に
