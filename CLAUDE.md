...
別フロアへ移動して戻った際 (保存フロアの復元時)、以前は
`reset_unique_by_floor_change()` (`src/floor/floor-changer.cpp`) が非ペットの
全モンスターの HP を最大値へ戻し、状態異常 (加速/減速/朦朧/混乱/恐怖/無敵) を
クリアしていた (= フロア再訪で全回復するバグ)。CreatureEntity 統合で HP・
時限効果が完全に保存・復元されるようになったため、この HP/状態リセット処理を
撤廃。保存フロア復元時はモンスターの状態が保存値のまま完全に維持される。
同関数の UNIQUE/NAZGUL の別フロア重複削除ロジックは残置。
**注意**: これにより不在中にモンスターが自然回復しなくなった (負傷状態が
そのまま保持される)。不在時間に応じた自然回復が必要なら別途
`absence_ticks` ベースの回復処理を `new_floor_allocation()` に実装すること。
