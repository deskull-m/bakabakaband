...
- **第 1 弾の付与 (13 体)**: いずれも `1_IN_1` / grade 0 / `1d1`。
  ...
  - **「二本の棍棒」「剣と盾」のような複数装備は 1 本だけにしてある**。利き手と逆手の
    両方を固定すると武装度予算をほぼ使い切ってしまい、他部位が極端に貧弱になるため。
  - **未着手の候補が約 95 体ある** (解説文に武具名があり `equip_kind` 未指定で装備可能な
    体構造の個体)。剣と盾・鎧兜の組合せ、ARCHER/RANGER 役割フラグ持ちの弓の個別指定、
    固有名の武器 (『エクスカリバーを短くしたような剣』等) は個別判断が要るため後続。
