...
レッドベレー 1828) に **軍服 (ベースアイテム id 788, tval 36 SOFT_ARMOR, AC6)** を
`1_IN_1` / grade 0 / `1d1` で付与した。5 種とも `body_structure` が `HUMANOID` なので
`wield_slot()` は `INVEN_BODY` を返し、**確実に胴へ装備される**。
...
- **`grade: 0` は素の品ではない**点に注意 (既存 `equip_kind` 28 エントリと同じ扱い)。
  `dun_level + 10` [%] でエゴ化・呪い・修正値の増減が起こるため、実機検証でも
  `軍服 [6] {呪われている}` が出た。**常に素の `軍服 [6]` にしたい場合は
  `"apply_magic": false` を足す**こと。
- 同シリーズの `軍帽` (id 789, tval 32 HELM) も存在するので、頭も揃えたい場合は
  同じ要領で追加できる。
