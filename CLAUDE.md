...
| `WINGED_HUMANOID` | 有翼人 | 全 13 スロットから**体の上 (クローク) を除く** (+ 拡張: 両翼の装飾) |
| `WING_ARMED_HUMANOID` | 翼腕人 | 全 13 スロットから**利き手・逆手を除く** (+ 拡張: 両翼の装飾) |
| `BIPEDAL` | 二足型 | 首・光源・胴・頭・脚 |
| `AVIAN` | 鳥型 | 首・胴・頭・脚 (+ 拡張: 両翼の装飾) |
| `QUADRUPED` | 四足型 | 首・胴・頭 |
| `INSECTOID` | 節足型 | 首・胴・頭 (`QUADRUPED` と同一構成) |
| `WORM` | 長胴型 | 首・胴 (+ 拡張: 尾の指輪) |
| `SERPENTINE` | 蛇型 | 首・胴 (+ 拡張: 尾の指輪) ※現在はコカトリスのみ |
...
- **`SPHERE` (球体)** は「手足も頭も首も無い、**球そのものが身体である存在**」用に
  追加した体構造。`INCORPOREAL` / `FORMLESS` / `GASEOUS` / `VORTEX` / `XAREN` と
  同じく**装備枠が一切ない**。五者の関係は `INCORPOREAL` = 実体なし /
  `FORMLESS` = 実体あり・形が不定 / `GASEOUS` = 実体が気体 /
  `VORTEX` = 回転・流動する現象 / **`SPHERE` = 形の定まった球体**。
- **適用済み: 17 体**。**17 体とも旧体構造は `INCORPOREAL`** なので
  **実挙動は一切変わらない** (思い出タグが `[非実体]` → `[球体]` になるだけの
  意味論の是正)。
  ...
  | 298 | 凍った球体 | 名前 |
  | 299 | 跳ねる火の玉 | 名前 (火の玉) |
  | 300 | 電気の球体 | 名前 +「スパークするエネルギーの球体」 |
  | 582 | ウィル・オー・ウィスプ | 「発光する奇妙な**光球**」 |
  | 912 | 宙に浮かぶ宝珠 | 名前 +「空中に浮かぶ金属製の宝珠」 |
  | 1336 | 美大ボール | 名前 |
  | 1422 | 『金のスニッチ』 | 「この**球体**を捕ったものは」 |
  | 1454 | ブラッジャー | クィディッチの鉄球 |
  | 1906 | マイアの戦闘球 | 名前 |
  | 1953 | ボール・ライトニング | 名前 (球電) |
  | 2040 | 重力の球体 | 名前 +「漆黒の球体」 |
  | 2063 | レギオン | 「その**球体**の内から触手と目玉が」 |
  | 2067 | 深淵の球体 | 名前 +「名状しがたい球体」 |
  | 2083 | 地獄の球体 | 名前 +「固まり集まった球体」 |
  | 2096 | 糞の球体 | 名前 +「汚臭を放つ糞の球体」 |
  | 2109 | 暗黒の球体 | 名前 +「闇そのものが球体になったもの」 |
  | 2110 | 閃光の球体 | 名前 +「光そのものが球体になったもの」 |

  - **判定基準は「名前か解説文が、その個体自身の形を球 (球体/球/玉/ボール/宝珠/
    光球) と述べているか」**。表示シンボル `*` 一括ではない。
  - **`*` の残り 16 体は対象外**。生首・顔面系 (目力先輩 1338 / 『This Man』1351 /
    涅槃先輩 1468 / 哀叫拓也 1772 / ラザロ 1776 / 巨大目力先輩 2060) は球ではなく
    頭部、星型・月型の ぷよ (2078 / 2079 / 2081)、鐘 (債務者の弔鐘 2119)、
    兎型ドローン (ウサウサストライカー 2152)、りんご (蠢くりんご 1476)、
    光体だが球と書かれない 黄色い光 (81)、形の記述が無い 死兆星 (1959) /
    スタビライザー (1982) / シグマウイルス (2316) はいずれも該当しない。
  - **`e` (目玉・ビホルダー) 系は対象外**。眼球は球ではあるが解説文は
    「体を持たない目玉」であって球体とは述べず、ビホルダーには触手もある。
    ガス・スポア (519) も「ビホルダーそっくりの外見」止まり。
  - **`E` のウィル・オー・ウィスプ (582) だけは `*` 以外から採った**。
    「発光する奇妙な**光球**」と球であることが明示されているため。ここは判断が
    入る線引きで、霊体扱いを優先するなら `INCORPOREAL` へ戻してよい
    (どちらでも挙動は同じ)。
  - **`&` の各種発生装置 (2097 / 2098 / 2099) も対象外**。球を発生させる
    「機械の箱」であって、自身は球ではない。
- **`WING_ARMED_HUMANOID` (翼腕人)** は「ハーピー・鴉人のように、**両腕そのものが
  翼になった**存在」用に追加した体構造。**`HUMANOID` から利き手・逆手だけを外した
  構成**で、拡張スロットは `WINGED_HUMANOID` と同じ両翼の装飾。武器も盾も持てない
  代わり、**背に翼を負うわけではないので体の上 (クローク) は羽織れる**
  (`WINGED_HUMANOID` とはここが逆になる)。
  - **利き手・逆手だけを外す最小構成にしてある**。射撃 (弓) と指輪は残しており、
    どちらも武装度充填の対象 7 部位に含まれないため生成には影響しない
    (`equip_*` で明示指定した場合のみ効く)。手で構えるものを全て外したければ
    `INVEN_BOW` / 指輪をマスクから追加で落とすこと。
- **適用済み: 3 体**。いずれも直前まで `WINGED_HUMANOID` だった。

  | id | 種族 | 旧体構造 (2 段階前) | 挙動の変化 |
  |---|---|---|---|
  | 51 | ホワイト・ハーピー | HUMANOID | **利き手・逆手を失う** (クロークは維持) |
  | 157 | ブラック・ハーピー | HUMANOID | **利き手・逆手を失う** (クロークは維持) |
  | 2014 | ベルカの鴉人 | BIPEDAL | **体の上・腕を得る** (利き手・逆手は無いまま) |

  - **ハーピー 2 体は明確な弱体化**。利き手は武装度充填の対象 7 部位の 2 番目で
    予算配分も大きいため、`HUMANOID` 時代に持っていた武器を失い近接ダメージが
    素手相当まで下がる。設定 (「女性の顔をした鳥」= 腕が翼) に合わせた是正。
  - **ベルカの鴉人は小幅な強化**。`BIPEDAL` のままではクローク・篭手も装備でき
    なかったため。ただし `WINGED_HUMANOID` 経由で一時的に得ていた利き手・逆手は
    本体構造では持たない (解説文が武器を持つとは述べていないため)。
  - **カイム (1555) は `WINGED_HUMANOID` のまま**。「翼から生えた**二本の腕**を
    持つ…**魔法のサーベルを携えている**」と、翼とは別に腕があり武器も持つため。
  - **`AVIAN` (鳥型) との違い**は「人型の胴体を持つか」。`AVIAN` は全身が鳥なので
    体の上・腕・光源・指輪も装備できないが、`WING_ARMED_HUMANOID` は胴から下が
    人型なのでそれらを装備できる。
- **`WINGED_HUMANOID` (有翼人)** は「有翼のデーモン・天使のように、人型の体に
  翼が生えた存在」用に追加した体構造。**`HUMANOID` から体の上 (クローク) だけを
  外した構成**で、拡張スロットとして `AVIAN` / `DRACONIC` と同じ両翼の装飾を持つ。
  背の翼が邪魔でマント・クロークの類を羽織れない一方、**手も脚も人型なので武器・
  盾・鎧・兜・篭手・靴は通常どおり扱える**。
- **これは実挙動を変える (小幅なバランス変更)**。`INVEN_OUTER` (体の上) は武装度
  充填 `get_armament_slot_entries()` の対象 7 部位に**含まれる**ため、`HUMANOID`
  から移した 40 体はクローク 1 枠ぶんの AC を失う (充填順は 胴 → 利き手 → 頭 →
  逆手 → 脚 → 腕 → 体の上 で体の上が最後なので、失われるのは繰り越し後の
  余り予算ぶん)。代わりにその予算は他部位へ回るため、総 AC の低下は 1 枠の
  実額よりは小さい。AVIAN / WORM / INSECTOID が「挙動不変の意味論是正」だったのに
  対し、**本体構造は意図的なバランス変更**である点に注意。
- **適用済み: 43 体**。

  | シンボル | 内容 | 体数 | 旧体構造 | 挙動の変化 |
  |---|---|---|---|---|
  | `A` | 天使 22 体 (天使〜熾天使 / ウリエル / ラファエル / アヴァシン 等) | 22 | HUMANOID | **クローク枠を失う** |
  | `U` | 有翼の大悪魔 8 体 (夜鬼 / ニカデーモン / クトゥルフ 等) | 8 | HUMANOID | **クローク枠を失う** |
  | `u` | 有翼の小悪魔 10 体 (インプ / クアシト / ガーゴイル / サキュバス 等) | 10 | HUMANOID | **クローク枠を失う** |
  | `V` | 吸血鬼の使者『スリングウェシル』 | 1 | HUMANOID | **クローク枠を失う** |
  | `B` | パズズ / カイム | 2 | BIPEDAL | **利き手・逆手・腕を得る** |

  - **`B` の 2 体だけは強化側**。いずれも `AVIAN` 移行時に「解説文が人間型の腕・体を
    明言している」として除外した個体で (パズズ=「羽を持った**人間型**の
    モンスター」、カイム=「翼から生えた**二本の腕**を持つ…**魔法のサーベルを
    携えている**」)、`BIPEDAL` のままでは武器を持てず設定と食い違っていた。
    **本体構造はその積み残しの受け皿**でもある。
  - **両腕そのものが翼になっている個体は `WING_ARMED_HUMANOID` (後述) へ分離した**
    (ハーピー 2 体 + ベルカの鴉人)。本体構造は「普通の人型の背中に翼が生えた者」
    専用で、手は人間と同じに残る。
  - **`A` の除外 5 体**: 自称天使『まひろ』(1455、天使を名乗る精神異常者) /
    「敵」の赤子 (1596、「ふわりと浮く」= 翼なし) / 『ピュアオディオ』(1613、
    概念的存在) / 創造神『KMR』(1633) / 新宿の『母』(1699、全裸の巨大な女性)。
    いずれも解説文に翼の記述が無く、天使であることが形態を意味しない個体。
  - **判定基準は「解説文が翼を明言しているか、または原典で翼が定義的か」**。
    `U` / `u` では解説文に翼・羽の記述がある個体 (夜鬼「蝙蝠の翼」/
    ストリガルドワー「灰色の鞣し革のような翼」/ ニカデーモン「翼を持ち」/
    渇血悪魔「羽根の生えた犬型」/ クトゥルフ系「背中に細長い翼」/
    スコトフォビア「翼と角の生えた男性の姿」/ ガーゴイル「羽の生えた」) と、
    原典で翼が定義的な archetype (インプ / クアシト / ホムンクルス /
    サキュバス / インキュバス / レッド・アリーマー / 鴉天狗) に限った。
  - **バルログ系 (720 / 839 / 856 / 940 / 1446 / 1663 / 1765) は対象外**。
    原典の翼の有無が定説として割れており、解説文も「全身が炎で覆われた屈強な
    人間型」としか述べていないため。同様に Doom 由来の悪魔 (サイバーデーモン /
    地獄の男爵 / マンキュバス / カコデーモン)・ヘル=ビースト・大悪魔
    (メフィストフェレス / アスモデウス / デモゴルゴン) も解説文に翼の記述が無く
    据え置いた。
  - **翼はあるが人型でない個体も対象外**。バイアクヘー (659、「頭部、胸部、腹部と
    いう昆虫のような体」) と忌まわしき狩人 (663、「巨大な黒い蛇か芋虫に似た
    生き物」) は `HUMANOID` のまま残した (前者は `INSECTOID` 相当だが、
    それはバランス変更になるため別途)。ヒポグリフ (209) / グリフォン (279) /
    マンティコア (472) / フェルダグリフ (1868) / ペガサス (990) /
    ドラコリスク (703) は有翼だが四足獣・竜なので該当しない。
- **`INSECTOID` (節足型)** は「外骨格と多数の脚を持つ節足動物」用に追加した
  体構造。**スロット構成は `QUADRUPED` と同一** (首・胴・頭) で、いわば
  **「四足型の節足動物版」**。昆虫・蛛形類・多足類・甲殻類をまとめて受けるため、
  トークンは設計書どおり `INSECTOID` だが**表示名は「昆虫型」ではなく「節足型」**に
  してある (蜘蛛・蠍・カニは昆虫ではないため)。設計書が挙げていた「多腕」拡張
  スロットは `ExtendedSlotType` に該当値が無いため未実装。
- **適用済み: 67 体**。

  | シンボル | 内容 | 体数 | 旧体構造 | 挙動の変化 |
  |---|---|---|---|---|
  | `c` | ムカデ 13 + 巨大カニ + バンダースナッチ 等 | 15 | SERPENTINE | **頭スロットを得る** |
  | `a` | 巨大アリ各種 + 『女王アリ』 | 9 | QUADRUPED | なし |
  | `K` | キラー・ビートル / 殺人クワガタ / 死番虫 等 | 13 | QUADRUPED | なし |
  | `S` | 蜘蛛・サソリ・ダニ (シェロブ / ウンゴリアント 等) | 30 | QUADRUPED | なし |

  - **`a` / `K` / `S` の 52 体はスロット構成が `QUADRUPED` と同一なので実挙動が
    一切変わらない** (思い出タグが `[四足型]` → `[節足型]` になるだけ)。
  - **`c` の 15 体だけは頭スロットを得る**ため兜を被るようになる (武装度予算の
    範囲内で小幅な AC 上昇)。**これは是正**で、ムカデに頭が無いわけではなく、
    元々「蛇型」に入っていたことの方が誤りだった。
  - **コカトリス (1084) だけ `SERPENTINE` に残した**。`c` シンボルだが節足動物では
    なく、鶏の体に蛇の尾という構成なので「首・胴 + 尾の指輪」が適合する。
    結果として **`SERPENTINE` の現在の所属はこの 1 体のみ**になった。
- **`I` (蟲の群れ 41 体) と `F` (トンボ等 9 体) は今回対象外**。どちらも現在
  `AMORPHOUS` (指輪のみ = 武装度充填の対象部位ゼロ) なので、`INSECTOID` へ移すと
  **胴と頭の装備を新たに得る=明確なバランス強化**になる。`I` は「蟲の大群」の
  ように単一の身体を持たない群体が多く `AMORPHOUS` にも理がある一方、
  **`F` の巨大トンボが「粘体型」なのは明らかに不整合**なので、バランス変更を
  許容できるタイミングで別途移行する余地がある。
- **`l` (魚・鯨・イカ 56 体) と `R` (トカゲ・恐竜 38 体) は節足動物ではない**ので
  対象外。`l` は設計書に未実装案として残る `AQUATIC` の候補。
- **`WORM` (長胴型)** は「ミミズ〜蛇〜東洋竜のような、手足を持たない長い胴が
  身体の主体である生き物」用に追加した体構造。**スロット構成は `SERPENTINE` と
  完全に同一** (首・胴 + 尾の指輪)で、これは「長い胴のみ」という括りが
  `SERPENTINE` のスロット構成そのものだったため。**新しいスロット構成の
  追加ではなく、`SERPENTINE` が担っていた意味論の実体を
  正しい名前の枠へ移す整理**である。
- **適用済み: `w` (蠕虫・軟体) 29 体 + `J` (蛇・ウナギ) 18 体 = 47 体**。
  47 体とも旧体構造は `SERPENTINE` なので**実挙動は一切変わらない**
  (思い出の表示タグが `[蛇型]` → `[長胴型]` になるだけ)。
  - `w` = イモムシの大群各色 / 巨大ヒル / 巨大ナメクジ / ドール /
    クトーニアン / 王蟲 / ワームとぐろエンジン / 甲鱗のワーム 等
  - `J` = 大白ヘビ / マムシ / キング・コブラ / 電気ウナギ / コアトル /
    ツチノコ / ヨルムンガンド / 『混沌のサーペント』 等
  - **東洋竜 (蛟・応龍の類) は現状 1 体も存在しない**。ワイアーム系 20 体は
    すべて四足の西洋竜で `DRACONIC`、三上山の『大ムカデ』(1088) は竜神ではなく
    ムカデ。`WORM` は「将来そこへ入れる枠」として用意してある状態。
  - **`蛇人間` (364) は境界例**。「直立した」と書かれており人型寄りだが、
    元から `SERPENTINE` で武器を持てない扱いだったため、挙動を変えないよう
    そのまま `WORM` へ移した。
- **`SERPENTINE` (蛇型) には多足類 16 体だけが残っている**。移行前の 63 体は
  `w` 29 + `J` 18 + **`c` 16 (ムカデ 13 / 巨大カニ / シャウラス 2 /
  コカトリス)** という内訳で、`c` 群は元々「蛇型」として明らかに不整合だった
  (ムカデは脚だらけ、巨大カニに至っては蛇ですらない)。設計書に未実装案として
  残る `INSECTOID` (多腕・多脚) を起こす際にまとめて再仕分けする方針で、
  **それまでの暫定置き場として `SERPENTINE` に残置**している。
  思い出では引き続き `[蛇型]` と表示されるが、これは移行前から同じ。
- **`AVIAN` (鳥型)** は「前肢が完全な翼になった鳥類」用に追加した体構造。
  `BIPEDAL` が「鳥型・恐竜型」を兼ねていたのを分離したもので、**`BIPEDAL` から
  光源スロットを外し、`DRACONIC` と同じ両翼の拡張スロットを加えた形**。
  翼で松明やランタンは掲げられないという理由で光源を落としてある。
  分離にあわせて `BIPEDAL` の説明は「二足型 (恐竜型・翼手型)」に改めた
  (残る 14 体はコウモリ 9 体 + 子泣きじじい等)。
- **実挙動は一切変わらない**。武装度充填 (`get_armament_slot_entries()`) が
  対象とするのは 胴・利き手・頭・逆手・脚・腕・体の上 の 7 部位で、
  **光源はそもそも対象外**。`BIPEDAL` ∩ 対象部位 = `AVIAN` ∩ 対象部位 =
  {胴・頭・脚} で完全に一致するため、移行した 39 体の装備生成は従来どおり。
  変わるのは思い出の表示タグが `[二足型]` → `[鳥型]` になる点と、
  `equip_kind` で光源を明示指定した場合に所持品送りになる点だけ
  (現状そのような指定を持つ鳥はいない)。
- **適用済み: 表示シンボル `B` の 43 体のうち 39 体**。全 43 体が `BIPEDAL` だった。

  | 除外した 4 体 | 理由 (解説文) |
  |---|---|
  | 827 風の帝王『パズズ』 | 「羽を持った**人間型**のモンスター」 |
  | 1555 地獄の論客『カイム』 | 「翼から生えた**二本の腕**を持つ…**魔法のサーベルを携えている**」 |
  | 2014 ベルカの鴉人 | 「細長い**人間の体**に鴉の頭と翼をもった異形の人」 |
  | 1924 呟く狂王『アーロン・ムスク』 | シンボルは鳥だが中身は人間 (企業家のパロディ) |

  - **判定基準は「解説文が人間型の腕・体を明言しているか」**。特にカイムは
    サーベルを携えると書かれており、利き手スロットを失うと設定と食い違う。
    この 4 体は `BIPEDAL` のまま残した (人型に寄せるなら `HUMANOID` だが、
    既存の挙動を変えないため据え置き)。
  - 機械・亡霊系 (BT-92 ブレナー 2157/2341、ハヤブサ 1146/1147) は
    「手を持たない飛行体」として `AVIAN` に含めた。
  - **`b` シンボルのコウモリ 9 体は対象外**。翼手類であって鳥類ではないため
    `BIPEDAL` のまま。
  - **`B` 以外の鳥らしきモンスターも対象外**にしてある。ハーピー (51 / 157)・
    ヒポグリフ (209)・グリフォン (279) は `H` (混成獣) で人型の腕を持つ扱い、
    コカトリス (1084) は `c` の `SERPENTINE`、森の『安藤』(1556) は
    ペンギンだが `t` (街の人間) の `HUMANOID`。いずれも既存分類を尊重した。
...
- **適用済み: クトゥルフ神話の不定形勢 12 体**。

  | id | 種族 | 旧体構造 | 解説文の根拠 |
  |---|---|---|---|
  | 848 | 森の黒山羊『シュブ=ニグラス』 | HUMANOID | 泡立ち、ただれた巨大な雲状の塊 |
  | 767 | ヴェールを与えるもの『ダオロス』 | HUMANOID | 形が入り組みすぎて目で確認できない |
  | 757 | 名状しがたきもの『ハスター』 | HUMANOID | 手足は骨がなくなって流動体のよう |
  | 714 | 双子のひわいなるもの『ツァール』 | QUADRUPED | 盛り上がった肉塊に無数の触手 |
  | 707 | 『ガタノソア』 | HUMANOID | 恐ろし過ぎて説明もできない姿 |
  | 662 | ツァトゥグァの落とし子 | HUMANOID | 不定形の黒いねばねば |
  | 1387 | ヨグ=ソトートの落とし子 | HUMANOID | 綱をよじり合わせたような触手と目玉 |
  | 845 | 全てにして一つのもの『ヨグ=ソトート』 | AMORPHOUS | 玉虫色の球の集合体 |
  | 833 | 不浄の源『アブホース』 | AMORPHOUS | 灰色がかった怖ろしい塊 |
  | 760 | ありうべからざるもの『ニョグタ』 | AMORPHOUS | 巨大な黒いアメーバ状 |
  | 685 | ショゴス | AMORPHOUS | 黒くネバネバした巨大な多細胞の原形質 |
  | 655 | 自存する源『ウボ=サスラ』 | AMORPHOUS | 泡立つ不定形の固まりに何百もの偽足 |

  - **旧 `HUMANOID` の 7 体が実挙動の変更**。剣も盾も篭手も装備できる状態だったのが
    構造的に不可になる。旧 `AMORPHOUS` の 5 体は指輪 2 枠を失うだけで、指輪は
    武装度の対象部位表に無いため**実挙動は変わらない**(意味論の是正)。
  - 旧 `HUMANOID` 側の 7 体は `armament_level: 0` も併せ持つ。体構造で既に装備不可
    なので**冗長だが残してある** (将来体構造を変えた場合の保険)。
  - **ショゴスは厳密には神格でなく「古のもの」の使役種族**だが、不定形の原形質
    そのものなので同時に移した。
  - `QUADRUPED` のまま残した蜘蛛・蛙・蛇型の神格 (アトラク・ナクア / ツァトゥグァ /
    イグ 等) は形が定まっているため対象外。これらは `armament_level: 0` のみで
    装備を止めている。
- **`GASEOUS` (気体)** は「毒ガス・煙・霧・大気そのものが体である存在」用に
  追加した体構造。`FORMLESS` と同じく**装備枠が一切ない**。
  `FORMLESS` が「実体のある塊だが形が定まらない」ものなのに対し、`GASEOUS` は
  **実体そのものが希薄な気体**である点が違う (三者の関係は
  `INCORPOREAL` = 実体なし / `FORMLESS` = 実体あり・形が不定 /
  `GASEOUS` = 実体が気体)。挙動は現状 3 者とも同じで、表示名と将来の分岐のために
  分けてある。
- **適用済み: 気体そのものが身体の 10 体**。**10 体とも旧体構造は `INCORPOREAL`**
  なので**実挙動は一切変わらない** (意味論の是正のみ)。

  | id | 種族 | 解説文の根拠 |
  |---|---|---|
  | 884 | 有毒ガス | 渦巻く黄色いガスの雲 |
  | 1250 | 可燃性ガス | 衝撃を与えると爆発する |
  | 365 | 吸血霧 | 意識を持った邪悪な霧 |
  | 625 | 不可解な煙霧 | 渦巻く螺旋状の霧で、常に形を変えている |
  | 2012 | コーラルの煙霧 | 増殖するエネルギー資源にして情報導体 |
  | 537 | 煙のエレメンタル | そびえ立つような漆黒の塊で、熱で火花を上げている |
  | 227 | 風のスピリット | 知能を持った旋風 |
  | 526 | 風のエレメンタル | そびえ立つような風の竜巻 |
  | 1141 | 年経た風のエレメンタル | あまりにも高くそびえ立つような風の竜巻 |
  | 687 | 風の女王『アリエル』 | そびえ立つような風の精霊 |

  - **風・大気系 4 体 (227 / 526 / 1141 / 687) を含めるのは判断の入る線引き**。
    空気は気体なので `GASEOUS` に入れたが、「ガス」と呼びたくないなら
    `INCORPOREAL` へ戻してよい (どちらでも挙動は同じ)。
  - **ボルテックス (`v`) 20 体は `GASEOUS` ではなく `VORTEX`** (後述)。火・水・
    冷気・破片・時間等、気体でないものを含む一群なので独立の体構造にしてある。
  - **ガス・スポア (519) / 霧の巨人 (552) は対象外**。前者は気体を詰めた球体、
    後者は名前に反して四本の附属肢を持つ塊で、どちらも形が定まっている。
- **`VORTEX` (ボルテックス)** は「渦・竜巻・奔流そのものが身体である存在」用に
  追加した体構造。`FORMLESS` / `GASEOUS` と同じく**装備枠が一切ない**。
  中身の属性 (火・水・冷気・破片・時間・混沌…) がばらばらでも、**「回転・流動する
  現象」という形態の共通項**で一群になるため、気体かどうかで割らずに独立の体構造に
  した (四者の関係は `INCORPOREAL` = 実体なし / `FORMLESS` = 実体あり・形が不定 /
  `GASEOUS` = 実体が気体 / `VORTEX` = 回転・流動する現象)。挙動は現状 4 者とも同じ。
- **適用済み: 表示シンボル `v` の全 20 体**。**20 体とも旧体構造は `INCORPOREAL`**
  なので**実挙動は一切変わらない** (意味論の是正のみ)。

  | id | 種族 |
  |---|---|
  | 354 / 355 / 358 / 359 | ファイア / ウォーター / コールド / エナジー・ボルテックス |
  | 587 / 588 / 599 / 600 | ネクサス / プラズマ / タイム・ボルテックス / 発光ボルテックス |
  | 751 / 752 / 897 / 1045 | カオス / エーテル / 破片 / 分解ボルテックス |
  | 1694 / 1695 / 1722 | ヴォイド / アビス / 糞便ボルテックス |
  | 682 | ロイガー (「力の"渦"として存在した」) |
  | 1370 / 1537 | 『いなずま・あらし』/ サメ台風 (嵐・台風) |
  | 1773 / 2100 | AI拓也 / クイックス (情報・線の奔流) |

  - **名前が「ボルテックス」でない 6 体 (682 / 1370 / 1537 / 1773 / 2100) も含めた**。
    シンボル `v` の一群として一貫させる方が扱いやすく、解説文もいずれも渦・嵐・
    奔流を述べているため。
- **`XAREN` (ザレン型)** は「岩の中を泳ぐ多腕多脚の異形」用に追加した体構造。
  手足自体は在るが、放射状に生えた腕と脚は人間用の武具の形に合わないため
  **装備枠が一切ない**。
- **適用済み: ザレン系 3 体**。**3 体とも旧体構造は `INCORPOREAL`** なので
  **実挙動は一切変わらない** (意味論の是正のみ)。
  ...
  - **同じ `X` シンボルの残り 4 体 (BB素材の残骸 1640 / 多脚ひで 1691 /
    トイボックス 2022 / 『バイオ兵器コア』 2269) は対象外**。ザレンの近縁では
    ないため `INCORPOREAL` のまま残した。**`XAREN` はシンボル一括ではなく
    ザレン系という種族のくくり**である点に注意。
...
- **`armament_level: 0` との使い分け**: 体構造は「構造的に装備できない」、
  ...
  体構造で表すほうが根本的**。
- **`AVIAN` の当初案との差**: 設計書
  (`docs/monster-body-structure-equipment-slots.md`) では `AVIAN` を 8 番に
  置く案だったが、`FORMLESS` 以降が先に末尾へ積まれたため**実際は 11 番**。
  enum 値は `body_slot_policies` の添字なので、設計書の番号ではなく
  実際の enum を参照すること。
...
`"armament_level": 0` を書くと `equip_monster_by_armament_budget()` が
`budget <= 0` で即 return するため、**武装度由来の装備が一切生成されなくなる**。
「体構造的には装備枠があるが、設定上およそ道具を身に着けない存在」に使う。

**適用済み: クトゥルフ神話の旧支配者・神格 19 体**。装備枠を持つ体構造
(`HUMANOID` / `QUADRUPED`) なのに解説文が明確に人型を逸しているものを選んである。

| id | 種族 | Lv | 体構造 |
|---|---|---|---|
| 857 | 『偉大なるクトゥルフ』 | 96 | HUMANOID |
| 848 | 森の黒山羊『シュブ=ニグラス』 | 91 | HUMANOID |
| 836 | クトゥルフの落とし子 | 86 | HUMANOID |
| 828 | 風に乗りて歩むもの『イタカ』 | 82 | HUMANOID |
| 1421 | ピカチャウ (ルルイエのすがた) | 80 | QUADRUPED |
| 814 | 蛇たちの父『イグ』 | 76 | QUADRUPED |
| 809 | 蜘蛛の神『アトラク・ナクア』 | 75 | QUADRUPED |
| 806 | ン・カイに眠るもの『ツァトゥグァ』 | 74 | QUADRUPED |
| 797 | 『ラーン=テゴス』 | 70 | QUADRUPED |
| 788 | 『グラーキ』 | 67 | QUADRUPED |
| 767 | ヴェールを与えるもの『ダオロス』 | 58 | HUMANOID |
| 757 | 名状しがたきもの『ハスター』 | 55 | HUMANOID |
| 714 | 双子のひわいなるもの『ツァール』 | 55 | QUADRUPED |
| 707 | 『ガタノソア』 | 46 | HUMANOID |
| 695 | 『ゾス=オムモグ』 | 45 | QUADRUPED |
| 681 | 丘からの恐怖『チャウグナール・ファウグン』 | 44 | QUADRUPED |
| 662 | ツァトゥグァの落とし子 | 41 | HUMANOID |
| 489 | 『ボクラグ』 | 33 | QUADRUPED |
| 1387 | ヨグ=ソトートの落とし子 | 32 | HUMANOID |

- **19 体とも役割フラグ (SOLDIER / WARRIOR / ARCHER / RANGER / MAGE) も
  `equip_*` 指定も持たない**ため、`armament_level: 0` だけで「装備一切なし」に
  なる。役割フラグや `equip_*` を持つ個体では、それらは別経路なので 0 にしても
  装備が残る点に注意。
- **`DROP_*` 系の一般ドロップは別機構**なので、倒したときのアイテムは従来どおり出る。
  変わるのは「生存中に身に着けている装備」と、その装備が死亡時に床へ落ちる分だけ。
- **`INCORPOREAL` / `AMORPHOUS` には書く必要が無い**。前者は全スロット不可、後者は
  指輪のみ可で指輪は武装度の対象部位表に無いため、**元から装備が生成されない**
  (アザトート / ヨグ=ソトート / クトゥグァ / ショゴス / ウボ=サスラ 等)。
- **人型・着衣が設定上重要な神格は対象外**にしてある。ナイアルラトホテップ (851) は
  「虹色のローブをまとい黄金の二重冠をいただく」、『黄衣の王』(735) は
  「襤褸を纏い青白き仮面をつけている」、ヒュプノス (787) は「ヒナゲシの冠」、
  ノーデンス (849) は「しわくちゃ老人のように見える」。ダゴン (595) /
  ハイドラ (596) は深き者の巨大化で半魚人型。
- **本来は `body_structure` の設定ミスに近いものも含む**。「泡立つ巨大な雲状の塊」
  (シュブ=ニグラス) や「不定形の黒いねばねば」(ツァトゥグァの落とし子) が
  `HUMANOID` なので剣も盾も持てる状態だった。`armament_level: 0` は武装度充填を
  止めるだけなので、より根本的に直すなら体構造を `AMORPHOUS` / `INCORPOREAL` へ
  変更する (ただし AC 計算や騎乗など他への波及があるため別途検討)。

**検証方法の注意 (実測で踏んだ罠)**: 「召喚して倒し、床に増えた装備品の数を数える」
方式で検証する場合、**`escorts` を持つ種族を選んではいけない**。ヨグ=ソトートの
落とし子 (1387) で測ったところ変更前後とも 7 個で変化が無く一見効いていないように
見えたが、実際には `escorts` で湧いた『混沌の獣人』を倒していただけだった
(撃破メッセージを出力して発覚)。`escorts` も `FRIENDS` も持たない
ツァトゥグァの落とし子 (662) で測り直すと、**変更前 7 個/体 → 変更後 0 個/体**と
明確に差が出る。

#### 武装度による装備枠の充填 (`equip_monster_by_armament_budget`)

武装度を**購買力**とみなし、生成時に装備スロットを埋める
(`src/monster-floor/monster-drop-generator.cpp`)。

- **対象スロット** (`get_armament_slot_entries()` の順に予算を割り当てる):
  胴 → 利き手 → 頭 → 逆手 → 脚 → 腕 → 体の上。
  指輪・首飾りは呪い付きや効果がランダムな品が多く、光源は燃料設定が要るため
  対象外。射撃 (弓) は射撃能力を持たない個体に持たせても無意味なので
  ARCHER / RANGER の役割装備に任せ対象外。
- **予算配分**: 残りスロット数で等分した額を各スロットの上限とし、余りは次の
  スロットへ繰り越す。1 部位に使い切らないため武装度が高いほど全身が上等になる。
- **選定基準は価値 (`cost`)**。予算内で最も高価な品の半額以上の候補から等確率で
  選ぶ。**性能 (AC / 打撃ダイス) を選定基準にしてはならない**: 本データには
  安価で高性能なフレーバー品 (`鉄の甲羅` AC22 が 1G、`サイコフレームヘルメット`
  AC26 が 75G 等) があり、性能で選ぶと予算が効かず Lv5 個体でも防具AC計 41〜86 に
  達する。実測比較は下表。性能値 (`calc_armament_quality`) は
  「装備しても無意味な AC0 の品」を候補から除く用途にのみ使う。

  | 選定基準 | Lv5 | Lv20 | Lv40 | Lv99 |
  |---|---|---|---|---|
  | 価値 (採用) | 防具AC計 17 / 武器ダイス 5 | 26 / 6 | 37 / 10 | 42 / 20 |
  | 性能 (不採用) | 41 / 5 | 79 / 6 | 80 / 16 | 86 / 24 |

- **候補の除外条件**: 生成確率が全て 0 の品 (通常フロアに出ない品)・
  固定アーティファクト化する品 (`INSTA_ART`)・クエスト専用品・呪い付きの品
  (`CURSED` / `HEAVY_CURSE` / `PERMA_CURSE`)・価値 0 の品・性能 0 の品。
- **役割装備との関係**: SOLDIER / WARRIOR の近接武器、ARCHER / RANGER の弓、
  MAGE の軽装で既に埋まったスロットはそのまま残し、その価値を予算から差し引く
  (二重取りしない)。よって**役割装備の 3 関数より後**、一般ドロップより前に呼ぶ。
- **バランス影響**: 装備は AC (`CreatureEntity::get_ac()`) と近接ダメージ
  (`calc_weapon_melee_damage`) に反映されるため、**装備可能な体構造を持つ全
  モンスターが強化される**。既定値 `level * 50` では防具AC計が Lv5 で +17 /
  Lv99 で +42 程度、近接武器を持たなかった個体も武器を持つようになる。
  死亡時は装備ごと床へ落ちるためアイテム供給も増える。調整は
  `ARMAMENT_LEVEL_PER_LEVEL` (既定 50) と対象スロット表で行う。
#### 固定アイテム指定: 生成時装備 (`equip_*`) と死亡時ドロップ (`drop_*`)
...
| JSON キー | 対象の指定 | 意味 |
|---|---|---|
| `equip_kind` | ベースアイテムID | **生成時**に持たせて装備させる。撃破すれば所持品ごと床へ落ちる |
| `equip_tval` | アイテム種別 | 同上。品は指定種別の中から無作為に選ばれる |
| `drop_kind` | ベースアイテムID | **死亡時に初めて生成**される。生存中は所持していない |
| `drop_tval` | アイテム種別 | 同上。品は指定種別の中から無作為に選ばれる |

```jsonc
"equip_kind": [
  { "id": 94, "probability": "1_IN_1", "grade": 0, "dice": "1d1" },
],
"equip_tval": [
  { "tval": 23, "probability": "1_IN_2", "grade": 1, "dice": "1d1" },
],
"drop_kind": [
  { "id": 750, "probability": "1_IN_1", "grade": 0, "dice": "1d5" },
],
"drop_tval": [
...
**択一指定 (`exclusive_group`, 4 キー共通)**: 複数の候補から**どれか 1 つだけ**を
出す場合に使う。既定では各エントリの確率は独立に抽選されるため、
`1_IN_2` を 2 つ並べても「両方出る 25% / どちらも出ない 25%」になってしまう。

```jsonc
"drop_kind": [
  { "id": 30, "probability": "1_IN_2", "grade": 0, "dice": "1d1", "exclusive_group": 1 },
  { "id": 47, "probability": "1_IN_1", "grade": 0, "dice": "1d1", "exclusive_group": 1 },
],
```

- 同じ番号を持つエントリは**リスト順に確率抽選し、最初に当たった 1 つだけ**を採用して
  同グループの以降のエントリはスキップする (**先勝ちカスケード**)。上例は
  「1/2 で折れたダガー、外れたら必ず折れた剣」= `randint1(2)` の 50/50 と等価。
- 3 択以上も同様に書ける (`1_IN_3` → `1_IN_2` → `1_IN_1` で均等 3 択)。
  末尾を `1_IN_1` にしなければ「どれも出ない」確率が残る。
- グループは**1 つのリストの中で閉じる**。`drop_kind` と `drop_tval` に同じ番号を
  書いても別グループとして扱われる。
- **同じ番号のエントリは連続して並べること**。実行時は離れていても択一として扱うが、
  思い出表示は直前のエントリと同じ番号のときだけ「さもなくば」で繋ぐため、
  飛び飛びに書くと表示と実挙動が食い違う。**非連続の再利用は読込時エラー**
  (`validate_exclusive_groups()`)。間に別のエントリを挟みたくなったら、
  そのエントリを択一の前後どちらかへ動かす。
- 省略時は 0 (グループ無し = 従来どおり独立抽選)。指定可能な範囲は 1〜100。
- `kill_interval` とは AND。`kill_interval` で弾かれたエントリはグループを消費しない。
- 思い出表示は「確率1/2で折れたダガーを1個、**さもなくば**折れた剣を1個落とす。」の形
  (グループ末尾の確率 1/1 は表記を省く)。

**魔法的強化の抑止 (`apply_magic`, 4 キー共通)**: `false` にすると `grade` を無視し、
`ItemMagicApplier` を**一切呼ばずベースアイテムのまま**生成する。

```jsonc
"drop_kind": [
  { "id": 30, "probability": "1_IN_1", "grade": 0, "dice": "1d1", "apply_magic": false },
],
```

- **`grade: 0` は「魔法無し」ではない**。`grade 0` は `ItemMagicApplier(..., dun_level,
  AM_NO_FIXED_ART)` を呼ぶので、`chance_good = dun_level + 10` [%] で power ±1、
  さらにその 2/3 の確率で power ±2 になり、**エゴ化・呪い・ダイス増加が起こる**。
  power 2 では 1/40 でランダムアーティファクト化まで起こりうる。
  「素のアイテムを落とす」ハードコーディングを忠実に移行するには `apply_magic: false`
  が要る。
- 省略時は true (従来どおり `grade` に応じた強化) で既存エントリの挙動は不変。
...
- `equip_*` = 「その個体が身に着けている / 持っている」。生成時に materialize され、
  装備できるスロットが空いていれば装備、体構造的に無理なら所持品に入る。
  **装備すれば AC (`CreatureEntity::get_ac()`) と近接ダメージ
  (`calc_weapon_melee_damage`) に反映される**。
- `drop_*` = 「倒して初めて得られる」。**生存中は存在しない**ので戦闘力に影響しない。
...
- 生成時装備は `equip_monster_fixed_items()`
  (`src/monster-floor/monster-drop-generator.cpp`) が `equip_kinds` / `equip_tvals` を
  走査して `acquire_item()` する。**呼出順は「生成時装備 → 役割装備 → 武装度充填」**で、
  種族固有の「らしさ」が最優先でスロットを確保し、装備した分の価値は武装度予算から
  差し引かれる。
- 死亡時ドロップは `drop_fixed_items_on_death()`
  (`src/monster-floor/special-death-switcher.cpp`) が `drop_kinds` / `drop_tvals` を
  走査して `drop_near()` する。`equip_*` は所持品として既に存在し
  `drop_all_inventory()` で落ちるため、この経路では**扱わない** (二重取りしない)。
- 等級 (grade) の魔法適用は両経路で `apply_drop_kind_magic()` を共用し、品質が
  変わらないようにしてある。
- `*_tval` から実際のベースアイテムを決めるのは
  `resolve_fixed_item_bi_id(entry, is_itemkind)`。**`BaseitemKey(tval)` (sval 省略)**
  で `lookup_baseitem_id()` の無作為抽選経路に入る。
  **`BaseitemKey(tval, 0)` と書いてはいけない**: これは「sval 0 のベースアイテム」の
  完全一致検索になり、SWORD 等 sval 0 が存在しない種別で例外を投げてゲームが落ちる
  (旧 `on_dead_drop_tval_item` がこの誤りを抱えていた)。
- **候補となるベースアイテムが 1 つも無い種別 (NONE / GOLD 等) は読込時にエラーで
  弾く** (`has_any_baseitem_of_kind()`)。データ不備を実行時の異常終了ではなく
  起動時のエラーにする。

**`*_tval` のバランス上の注意**: 抽選は当該種別の**全 sval から一様**で、モンスターの
レベルも武装度も考慮しない。`"tval": 23` (刀剣) と書くと Lv12 の個体が
エスパドン (2d9) を装備することもある。格を揃えたい場合は `*_kind` で個別指定するか、
将来的に武装度予算で絞る仕組み (`pick_armament()` の流用) を入れること。

思い出表示は 4 リストとも `display_drop_entries()` テンプレートで共通化し、
導入句と結びだけを変えている (`equip_*` = 「〜を装備している。」/ `drop_*` =
「倒すと、〜落とす。」)。`*_tval` は個別のアイテム名ではなく**種別名**で表示する
(名称はアイテム生成ウィザードの `tval_desc_list` を共用。表に無い
NONE / GOLD / BOTTLE / NO_AMMO は総称「アイテム」へフォールバック)。

**実データの移行**: 旧仕様では `drop_kind` のうち装備品を `is_wearable_drop_kind()`
(ベースアイテムの tval で判定) で自動的に生成時装備に回していた。本整理で
**意味をキー名で明示**する方針にしたため、この暗黙の振り分けは廃止し、
当時装備されていた 28 エントリ (25 体) を `equip_kind` へ移した
(例: ボウガンのガキ → 硬革帽子 + ライト・クロスボウ、汚いイタズラ小僧 → 汚れたボロ服)。
装備品でない 63 エントリは `drop_kind` に残る。合計 91 エントリは不変で、
**挙動も移行前と同一**。拳王親衛隊『ザルカ』/『カシム』のように装備品と非装備品を
併せ持つ個体は 2 キーに分割された (首切りソード = `equip_kind`、「あべし」「ひでぶ」
= `drop_kind`)。

**制服・組織装備の付与 (アライアンス単位のフレーバー装備)**: 特定の勢力に属する
モンスター群へ「その組織の制服」を `equip_kind` で一括付与していく方針。
第 1 弾としてアライアンス `GOLAN` の全 5 種
(総帥『カーネル』1607 / 軍曹『マッド』1608 / GOLAN兵 1687 / GOLAN将校 1692 /
レッドベレー 1828) に **軍服 (id 788, tval 36 SOFT_ARMOR, AC6)** と
**軍帽 (id 789, tval 32 HELM, AC3)** を `1_IN_1` / grade 0 / `1d1` /
`apply_magic: false` で付与した。5 種とも `body_structure` が `HUMANOID` なので
`wield_slot()` はそれぞれ `INVEN_BODY` / `INVEN_HEAD` を返し、**確実に胴と頭へ
装備される**。

- **付与対象の見つけ方**: `lib/edit/MonraceDefinitions.jsonc` の
  `"alliance": "<名前>",` で grep する。`src/alliance/alliance-<名前>.cpp` の
  `MonraceId` 参照と件数が一致するか突き合わせると漏れが無い。
- **武装度予算との関係に注意**: 固定装備は武装度充填 (`equip_monster_by_armament_budget`)
  より**先**に走ってスロットを確保し、その価値が予算から差し引かれる。軍服は
  価値 3G と安いので、**胴スロットを安価な制服が占有し、本来そこへ充てられた
  予算 (Lv16 の GOLAN兵 なら約 114G) が他スロットへ繰り越される**。防具 AC の
  総和はやや下がる代わりに頭・脚・腕が上等になる、というトレードオフになる。
  これは「種族固有のらしさを最優先」という設計どおりの挙動。
- **支給品には `"apply_magic": false` を付ける**。`grade: 0` は素の品ではなく
  `dun_level + 10` [%] でエゴ化・呪い・修正値の増減が起こるため、指定前の実機検証では
  `軍服 [6] {呪われている}` や `{高級品}` が出ていた。制服は組織の支給品であって
  戦利品ではないので、`apply_magic: false` で素の `軍服 [6]` / `軍帽 [3]` に固定する
  (既存 `equip_kind` 28 エントリは既定の強化ありのまま。制服系のみこの扱い)。
- **種族名と食い違う場合はベースアイテムを新設して差し替える**。「GOLANのレッドベレー」
  (1828) だけは軍帽ではなく **赤いベレー帽 (id 830, tval 32 HELM, sval 28, AC3)** を
  被る。これは本件のために `lib/edit/BaseitemDefinitions.jsonc` へ新規追加した頭部
  装備で、既存の頭部装備と同じ書式 (level 25 / weight 10 / cost 30 / base_ac 3 /
  allocations depth 25 rarity 2) で末尾に追記してある。
- **ベースアイテムの新設は末尾追記なら後方互換**。savefile は
  `ItemLoaderBase::load_item()` が鑑定情報の件数をファイルから読み、
  `i < baseitem_records.size()` を超える分は dummy へ捨てるため、**件数が増えても
  旧セーブはそのまま読める** (新規アイテムが未鑑定状態で始まるだけ)。
  セーブデータバージョンの更新は不要。
...
- **固定アーティファクト専用のベースアイテムは `INSTA_ART` + `allocations` 省略**。
  ...
  グロンドのベース「冥界の大鉄杖」(498) と同じ形で `flags` に `INSTA_ART` を持ち
  **`allocations` を一切書かない**。これにより:
  - 素の「黄金の槍」が通常のアイテム生成に出てくることは**一切ない**
    (アロケーションテーブルに載らないため)。
  - 代わりに `ArtifactType::try_make_instant_artifact()` 経由で**アーティファクトとして
    直接生成**される。発生率はアーティファクト側の `rarity` (`one_in_(rarity)`) と
    `level` が決め、269 は `rarity: 255` (スキーマ上限・データ内最大) / `level: 110` で
    自然生成は極めて稀。実質メルコール撃破が入手経路になる。
  - `equip_monster_by_armament_budget()` は `INSTA_ART` 付きベースアイテムを候補から
    除外する (`monster-drop-generator.cpp`) ため、武装度充填で他のモンスターが
    持つこともない。
  - **検証**: ウィザードの `c` (アイテム生成) で当該ベースアイテムを選ぶと
    `wiz_create_item()` が bi_key 一致の固定アーティファクトを探して
    `Allocated(INSTA_ART).` と出す。実測で
    `★冥王メルコールの大槍 (6d9) (+32,+20) [-10] (+3)` が生成されることを確認済み。
  - **アーティファクトの `base_item` 変更はセーブデータ非互換にならない**
    (アーティファクト定義は JSON 由来でセーブに載らない) が、**既に生成済みの
    当該アーティファクトを所持しているセーブでは見た目のベース名が変わる**。

**フレーバー整合の付与 (解説文が名指しする武具を実際に持たせる)**: 種族解説文が
「〜を持っている」「〜を振り回す」と具体的な武具を名指ししているのに、システム上は
何も持っていない (= 武装度充填が価値基準で選んだ任意の品を持つ) 個体が多数ある。
これを `equip_kind` で埋めていく方針。**解説文が既に書いていることをデータ化する
だけ**なので、フレーバーの新規創作は伴わない。

- **確率は解説文の断定度に合わせる**。「斧を持っている」のように断定している個体は
  `1_IN_1`。「〜のこともある」程度なら `1_IN_5` 等。第 1 弾では、解説文が断定
  しているのに `1_IN_5` だった **きこり (6, ハチェット) / 漁師 (2120, スピア)** を
  `1_IN_1` に是正した。
- **`apply_magic` は既定 (強化あり) のまま**。制服・支給品 (GOLAN) と違い、個人の
  得物なので深度なりの強化が付くのはむしろ自然。
- **武装度予算との関係**: 固定装備が利き手スロットを先に確保し、その価値が予算から
  差し引かれる。よって「任意の高価な武器」が「解説文どおりの武器」に置き換わるだけで、
  総戦力は上下どちらにも小幅に振れる (解説文の武器が高価なら他スロットが貧弱になる)。
- **第 1 弾の付与 (13 体)**: いずれも `1_IN_1` / grade 0 / `1d1`。

  | id | 種族 | Lv | 解説文の記述 | 付与 |
  |---|---|---|---|---|
  | 1166 | チーターマン『アリエス』 | 11 | 棍棒を振り回している | 棍棒 (542) |
  | 1168 | チーターマン『アポロ』 | 15 | クロスボウからの一撃 | ライト・クロスボウ (75) |
  | 1488 | 葦名流門弟 | 17 | 戦国の剣術を学んだ素肌剣術の武者 | カタナ (41) |
  | 901 | サムライ | 25 | 剣をピクリとも動かさずに構え | カタナ (41) |
  | 511 | 智天使 | 33 | 聖なるウォーハンマーを手に持ち | ウォー・ハンマー (55) |
  | 1127 | 緑鬼 | 36 | 灼けた斧で劫罰を与え続ける | バトル・アックス (68) |
  | 1416 | 狂戦士『バストラル』 | 37 | 伝説のバトル・アックス、ウィンドバーグ | バトル・アックス (68) |
  | 1898 | 断罪者『ミラルダ』 | 37 | 重い首切り斧を平然と振り回す | バトル・アックス (68) |
  | 1787 | 牛頭のデーモン | 38 | 大斧を奮う牛の頭をしたデーモン | グレート・アックス (67) |
  | 1914 | 聖騎士『リロイ』 | 39 | 神聖なる力を帯びた巨大な鉄鎚 | グレート・ハンマー (399) |
  | 631 | 闘トロル | 40 | シミターと重い鎧を好んで使う | シミター (32) |
  | 637 | アーチプリースト | 40 | 刺の付いたメイスを狂ったように打ちつけ | モーニング・スター (52) |
  | 648 | 撲殺の悪魔 | 40 | 二本の棍棒を乱暴に振り回す | 棍棒 (542) |

  - **13 体とも役割フラグ (SOLDIER / WARRIOR / ARCHER / RANGER / MAGE) を持たない**
    ため、これまで利き手は武装度充填任せだった。付与により解説文どおりの得物になる。
- **第 2〜4 弾で残りを一巡 (85 体)**。第 2 弾 = Lv0-29 の 22 体、第 3 弾 = Lv30-49 の
  39 体、第 4 弾 = Lv50 以上の 24 体。**合計 100 体に `equip_kind` が付いた**
  (第 1 弾 13 + 確率是正 2 + 85)。個別の対応表はコミットメッセージを参照。
  - **複数装備は第 2 弾以降で解禁**。「剣と弓」「大剣と大盾」「鎧と兜」「ローブと杖」
    のように解説文が 2 点を名指しする場合は 2 点とも付ける。ただし**合計価値が武装度
    予算 (`level * 50`) に収まる品を選ぶ**のが原則で、収まらない場合は主武器 1 点に絞る。
  - **予算超過を許容した例外は 2 体だけ**。暗黒城の魔術師『アンサロム』(1005) の
    魔術師の杖 (4000G / 予算 2750G) と混沌の覇者『はぶ』(770) のカオス・ブレード
    (4000G / 予算 3050G)。いずれも固有名のユニークで、その得物が個体の本体そのもの
    なので超過を受け入れた (**超過しても生成は成功する**。固定装備は武装度充填より
    先に無条件で付与され、予算が負になると他部位が空になるだけ)。
  - **既存の専用ベースアイテムは拾い上げる**。オーラントの公使 (1092) には
    「オーラントの公使の帽子」(709) が既にあり、解説文と完全に一致するため
    そのまま付与した。
  - **`INSTA_ART` のベースアイテム (☆専用で通常生成されない品) は装備させない**。
    冥王『メルコール』(861) は解説文が「冥界の大鉄杖『グロンド』」を名指しするが、
    そのベースアイテム「冥界の大鉄杖」(498) は `INSTA_ART` 付き・`allocations` 空で
    **生成すれば必ず☆になる品**であり、かつ**グロンドは『混沌のサーペント』(862) の
    `artifacts` ドロップ専用**とする方針なので装備させない (素で持たせると
    「偽グロンド」になる)。**メルコールには代わりに自身の固定アーティファクト
    「冥王メルコールの大槍」(artifact 269) を `artifacts` ドロップ (確率 100) で
    割り当てた**。同アーティファクトのベースとして新設した**黄金の槍 (id 831)** も
    `INSTA_ART` 付き・`allocations` 空なので (上記「固定アーティファクト専用の
    ベースアイテム」参照)、**これ自体を `equip_kind` へ書くことはしない**。
  - **一方「たまたま☆のベースでもある通常アイテム」は気にせず付与してよい**。
    メルコールの「鉄冠」(98) は固定アーティファクト「ベルシエル王妃の鉄冠」
    (artifact 35) のベースだが、`allocations` (depth 40 / rarity 1) を持つ通常生成品で
    `INSTA_ART` も無く、`grade` 0〜2 は `AM_NO_FIXED_ART` 付きなので
    **☆にはならず素の鉄冠として装備される** (☆側も通常どおり入手できる)。
    ロング・ソード (11 個の☆のベース) / ダガー (10 個) / カタナ (9 個) / ローブ /
    クローク / 軍服 も同様で、**`equip_kind` 109 エントリ中 82 が何らかの☆のベース**に
    あたる。これを除外条件にするとありふれた武具がほぼ全て使えなくなるため、
    判定は上記のとおり `INSTA_ART` の有無で行う。
  - **新規ベースアイテムが要るため見送った 2 体**: 黒衣の騎士『アシュラム』(974) の
    「魔剣」、哀しみの『レイべン』(1602) の「特殊合金のマント」。該当する品が無く、
    近縁品での代替は解説文の核を損なうため保留 (赤いベレー帽と同じく新設で対応可能)。
  - **対象外と判断した約 10 体**: 解説文の武具名がプレイヤー側の装備を指すもの
    (デイドラ・ブラック・ドラゴン 1066「生半可な鎧では」)、ジャンルの比喩
    (ネクロマンサー『アビゲイル』1991「剣と魔法のファンタジー世界」)、ゲーム用語
    (『汚い忍者』1939「空蝉盾」)、爪牙や素手のみの個体 (グレート・クリスタル・
    ドレイク 1201 / 超電磁空手の達人『刀耳』2284) 等。

**貧窮層への「汚れたボロ服」付与 (街の `t` 貧乏人)**: 表示シンボル `t` (街の人間)
のうち**設定上あきらかに困窮している個体**に **汚れたボロ服 (id 102, tval 36
SOFT_ARMOR, sval 1, AC1, 1G)** を `equip_kind` で与える。制服 (GOLAN / FRIEZA-CLAN)
が「組織の支給品」なのに対し、こちらは「その人物が着ている唯一の衣服」という
フレーバー整合の付与にあたる。

- **新規付与 10 体** (いずれも `1_IN_1` / grade 0 / `1d1`)。

  | id | 種族 | Lv | 根拠 |
  |---|---|---|---|
  | 11 | わめくキチガイ | 0 | 街の浮浪者一群 (9/10/12/13 と同じ RABBLE) |
  | 15 | 歌を口ずさむ幸せな酔っぱらい | 0 | 同上。街をふらつく酔漢 |
  | 17 | 貧しげな傭兵 | 0 | 名前が「貧しげな」+「仕事が見つからないことが彼をいっそう貧しくしている」 |
  | 1286 | レッサー不潔で醜い哀れな身なりの…キチガイ人間 | 0 | 名前に「不潔」「哀れな身なり」 |
  | 1393 | 老人『ミスミ』 | 0 | 食糧難の村へ種もみを運ぶ老人 |
  | 1738 | 薬中 | 0 | 幻覚の中で戦う薬物中毒者 |
  | 1812 | バル・ベルデ市民 | 0 | 「過酷極まる政情不安定な国家の民…多くが荒みきっている」 |
  | 2007 | コーラル中毒者 | 0 | 「ルビコン3の過酷な環境を奇跡的に生き延びてきた中毒者」 |
  | 2044 | アンパンマン（アレの方） | 0 | シンナー中毒の浮浪者 |
  | 2068 | ヘルジャパンからの亡命者 | 0 | 「逃亡してきた」「生まれついての被害者」 |

- **既存 8 体の確率を `1_IN_5` → `1_IN_1` に是正**した (1 汚いイタズラ小僧 /
  9 泣きじゃくる白痴 / 10 醜い浮浪者 / 12 哀れな身なりの乞食 / 13 不潔な病人 /
  1822 ホモガキ / 1870 拓也県民 / 1882 肉便器女)。この `1_IN_5` は旧 `drop_kind`
  からの機械的移行で付いた値で、**「浮浪者」「乞食」「哀れな身なり」と名前が断定
  している以上 5 体に 4 体が全裸になるのは不整合**。きこり (6) / 漁師 (2120) の
  確率是正と同じ判断基準 (「解説文の断定度に合わせる」) に沿う。
- **バランス影響はほぼ皆無**。18 体とも Lv0〜Lv0 で武装度予算
  (`level * ARMAMENT_LEVEL_PER_LEVEL` = 0) が 0 のため
  `equip_monster_by_armament_budget()` は即 return しており、そもそも充填装備を
  一切持っていなかった。付与されるのは AC1 / 価値 1G の最下級防具 1 枚だけで、
  床へ落ちる分も無価値。
- **`apply_magic` は既定 (強化あり) のまま**。既存 8 体と書式を揃えるためで、
  Lv0 の街では `chance_good = dun_level + 10` が 10% 程度と低く、まれに
  `{呪われている}` 等が付いてもボロ服らしさを損なわない。
- **対象外にした主な個体**: 全裸と明記されている者 (『INUE君』1337 /
  教育実習生『神谷』1622)、露出狂 (1736 / 2104)、勤め人 (マケグミ・サラリマン
  1645 / カチグミ・サラリマン 1646 — 貧しくてもスーツを着ている)、
  街のクズだが困窮していない者 (チンピラ 1618 / 取り立て人『HND△』2246)、
  全身が合金製と書かれている『ひで』(1356)、街ではなく森に出る水生の村民 (1495)。

**検証方法 (フレーバー付与の確認)**: 思い出に「確率1/1で〜を装備している。」が出ることを
確認したうえで、**`ONLY_GOLD` を持つ種族で実際に床へ落ちるかを測る**のが決定的。
`ONLY_GOLD` の個体は一般ドロップで道具を落とさないため、**倒した後に該当 tval の品が
床に現れたら、それは装備由来と確定できる** (きこり 4 体を倒して POLEARM が 4 個出現、
召喚前の周辺 POLEARM は 0 個)。`ONLY_ITEM` / `DROP_*` を持つ種族 (サムライ等) では
一般ドロップと区別が付かないので、この測り方は使えない。

**ハードコーディングからの移行例 (<コーン>の血戮悪魔)**: 特定モンスターの固定
アイテムが C++ に直書きされている場合、この 4 キーへ移せる。血戮悪魔は
`on_dead_bloodletter()` が死亡時に 15% でカオス・ブレードを生成していたが、
種族解説文が「ヘルブレードを装備していて」と述べるとおり**本来は装備している**もの
なので、`equip_kind` (id 36 = カオス・ブレード, `15_IN_100`, grade 0, `1d1`) へ移し、
`special-death-switcher.cpp` の関数と `MonraceId::BLOODLETTER` の case を削除した。

- 旧実装の `ItemMagicApplier(..., AM_NO_FIXED_ART | md_ptr->mo_mode)` は、この種族が
  `DROP_GOOD` / `DROP_GREAT` / `DROP_NASTY` をいずれも持たず `mo_mode == 0` のため
  **grade 0 と等価**。
...
**ハードコーディングからの移行例 (折れたデスソード)**: `B_DEATH_SWORD` (id 953) の
case が `ItemEntity item({ SWORD, randint1(2) })` で折れたダガー / 折れた剣を
50/50 で落としていたものを `drop_kind` 2 件 (id 30 = 折れたダガー `1_IN_2`、
id 47 = 折れた剣 `1_IN_1`。いずれも grade 0, `1d1`, `exclusive_group: 1`,
`apply_magic: false`) へ移した。**シンボルが `|` でミミック分岐に該当する**ため、
本移行にあわせて `on_dead_mimics()` に抑止条件を足している (下記)。

- **`exclusive_group` が必須**: `randint1(2)` は択一なので、独立抽選のままでは
  「両方落ちる / どちらも落ちない」が各 25% 生じる。先勝ちカスケードで
  「1/2 でダガー、外れたら必ず剣」と書くと旧実装と完全に同じ分布になる。
- **`apply_magic: false` が必須**: 旧実装は `ItemMagicApplier` を一切呼ばず素の
...
- **既知のフレーバー上の粗**: 武装度充填は価値基準のため、高価だが弱いネタ装備
  (`サングラス` 500G、`伸縮性のあるボクサー型のパンツ` 200G 等) が選ばれることが
  ある。気になる場合は当該ベースアイテムの候補除外か価値の見直しで対応する。
