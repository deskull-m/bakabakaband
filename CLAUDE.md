...
#### 装備品の固定ドロップを生成時に装備させる (`equip_monster_fixed_drops`)

「その装備を落とす」と定義されたモンスターは本来それを**身に着けている**のが
自然なため、固定ドロップ (`MonraceDefinition::drop_kinds`) のうち装備品は
死亡時ではなく**生成時**に materialize して装備スロットへ入れる。

- **振り分けの述語は `is_wearable_drop_kind(bi_id)`** (`BaseitemKey::is_wearable()`)。
  生成時に持たせる側と死亡時に生成する側 (`on_dead_drop_kind_item`) が
  **同一の述語をベースアイテムIDのみで**評価するため、判定が食い違って
  二重ドロップ・ドロップ消失が起きることはない。装備品でない固定ドロップは
  従来どおり死亡時に生成される。
- **等級 (grade) の魔法適用は `apply_drop_kind_magic()` に共通化**し、生成時と
  死亡時で品質が変わらないようにした (死亡時処理の switch を移設)。
- 体構造的に装備できない・スロットが埋まっている場合は所持品に入り、いずれにせよ
  死亡時は `drop_all_inventory()` で床へ落ちる。
- **呼出順は「固定ドロップ → 役割装備 → 武装度充填」**。種族固有の「らしさ」を
  最優先してスロットを確保させ、装備した分の価値は武装度予算から差し引かれる。
- 実データでは固定ドロップ 91 エントリ中 27 が装備品
  (例: ボウガンのガキ → 硬革帽子 + ライト・クロスボウ を `1_IN_1` で装備)。
...
- **固定ドロップの思い出表示が出ない不具合も修正済み**: `display_drop_kind_items()`
  は `display_monster_drops()` の末尾で呼ばれていたが、同関数の冒頭に
  「一般ドロップ (`drop_gold` / `drop_item`) が両方 0 なら早期 return」があり、
  **実データの `drop_kind` 保有 57 体は全て一般ドロップを持たない**ため、
  固定ドロップの思い出文 (「〜は倒すと、確率1/1で〜を1d5個落とす。」) は
...
- **既知のフレーバー上の粗**: 価値基準のため、高価だが弱いネタ装備
