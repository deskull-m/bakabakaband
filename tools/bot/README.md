...
```sh
mkdir -p /tmp/hbsave
src/bakabakaband -mgcu --control-port=9000 --fixed-seed=12345 \
...
```bat
Bakabakaband.exe --control-port=9000 --fixed-seed=12345
```
...
```bat
rem 既存のセーブをロードする
Bakabakaband.exe --headless --control-port=9000 lib\save\PLAYER

rem 新規キャラクターから始める (位置引数を渡さない)
Bakabakaband.exe --headless --control-port=9000 --fixed-seed=12345
```
...
```sh
mkdir -p /tmp/hbsave
src/bakabakaband --headless --control-port=9000 --fixed-seed=12345 \
