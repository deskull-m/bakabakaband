...
def build_validation_pairs(edit_dir: Path, schema_map: dict[str, Path], loaded_schemas: dict[Path, dict]) -> list[tuple[Path, Path, dict]]:
    ...
    for data_file in sorted(edit_dir.glob("*.jsonc")):
        ...
        if not schema_file:
            # bakabakaband 独自仕様: 対応するスキーマが未整備の jsonc は
            # 検証対象から除外する (CI を失敗させずスキップ扱いとする)。
            skipped.append(data_file.name)
            continue

        pairs.append((data_file, schema_file, loaded_schemas[schema_file]))

    # 固定クエストは1ディレクトリ下に個別ファイルとして存在し、全て単一の "Quest" スキーマを共有する。
    ...
    if skipped:
        print(f"Skipped (no matching schema): {', '.join(skipped)}")
...
def main():
    ...
    print("Bakabakaband JSONC validation started! Loading schemas...")
